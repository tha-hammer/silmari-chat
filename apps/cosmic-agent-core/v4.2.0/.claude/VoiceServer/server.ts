  ...
  async fetch(req) {
    ...
    if (url.pathname === "/notify" && req.method === "POST") {
      try {
        ...
        const title = data.title || "AAI Notification";
    ...
    if (url.pathname === "/notify/personality" && req.method === "POST") {
      try {
        ...
        await sendNotification("AAI Notification", message, true, null);
    ...
    if (url.pathname === "/pai" && req.method === "POST") {
      try {
        const data = await req.json();
        const title = data.title || "AAI Assistant";
        const message = data.message || "Task completed";

        console.log(`🤖 AAI notification: "${title}" - "${message}"`);

        await sendNotification(title, message, true, null);

        return new Response(
          JSON.stringify({ status: "success", message: "AAI notification sent" }),
          {
            headers: { ...corsHeaders, "Content-Type": "application/json" },
            status: 200
          }
        ...
        console.error("AAI notification error:", error);
        return new Response(
          JSON.stringify({ status: "error", message: error.message || "Internal server error" }),
          {
            headers: { ...corsHeaders, "Content-Type": "application/json" },

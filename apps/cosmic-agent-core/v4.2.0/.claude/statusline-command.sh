...
SETTINGS_FILE="$AAI_DIR/settings.json"
RATINGS_FILE="$AAI_DIR/MEMORY/LEARNING/SIGNALS/ratings.jsonl"
TREND_CACHE="$AAI_DIR/MEMORY/STATE/trending-cache.json"
MODEL_CACHE="$AAI_DIR/MEMORY/STATE/model-cache.txt"
QUOTE_CACHE="$AAI_DIR/.quote-cache"
LOCATION_CACHE="$AAI_DIR/MEMORY/STATE/location-cache.json"
WEATHER_CACHE="$AAI_DIR/MEMORY/STATE/weather-cache.json"
USAGE_CACHE="$AAI_DIR/MEMORY/STATE/usage-cache.json"
...
COUNTS_CACHE="$AAI_DIR/MEMORY/STATE/counts-cache.sh"
...
# NOTE: Claude Code uses lowercase "projects/" dir, AAI uses uppercase "Projects/".
SESSION_LABEL=""
SESSION_NAMES_FILE="$AAI_DIR/MEMORY/STATE/session-names.json"
SESSION_CACHE="$AAI_DIR/MEMORY/STATE/session-name-cache.sh"
if [ -n "$session_id" ]; then
    # Derive sessions-index path from current_dir (Claude Code uses lowercase "projects")
    project_slug=$(echo "$current_dir" | tr '/.' '-')
    SESSIONS_INDEX="$AAI_DIR/projects/${project_slug}/sessions-index.json"
...
_parallel_tmp="/tmp/aai-parallel-$$"
...
_width_cache="/tmp/aai-term-width-${KITTY_WINDOW_ID:-default}"
...
# NOTE: DA_NAME, AAI_VERSION, input JSON, cc_version, model_name
...
# AAI Branding (matches banner colors)
AAI_P='\033[38;2;30;58;138m'          # Navy
AAI_A='\033[38;2;59;130;246m'         # Medium blue
AAI_I='\033[38;2;147;197;253m'        # Light blue
AAI_LABEL='\033[38;2;100;116;139m'    # Slate for "status line"
AAI_CITY='\033[38;2;147;197;253m'     # Light blue for city
AAI_STATE='\033[38;2;100;116;139m'    # Slate for state
AAI_TIME='\033[38;2;96;165;250m'      # Medium-light blue for time
AAI_WEATHER='\033[38;2;135;206;235m'  # Sky blue for weather
AAI_SESSION='\033[38;2;120;135;160m'  # Muted blue-gray for session label
...
# ═══════════════════════════════════════════════════════════════════════════════
# LINE 0: AAI BRANDING (location, time, weather)
# ═══════════════════════════════════════════════════════════════════════════════
...
# Output AAI branding line
case "$MODE" in
    nano)
        printf "${SLATE_600}── │${RESET} ${AAI_P}A${AAI_A}A${AAI_I}I${RESET} ${SLATE_600}│ ────────────${RESET}\n"
        printf "${AAI_TIME}${current_time}${RESET} ${AAI_WEATHER}${weather_str}${RESET}\n"
        printf "${SLATE_400}ENV:${RESET} ${SLATE_500}${AAI_A}${AAI_VERSION}${RESET} ${SLATE_400}ALG:${AAI_A}${ALGO_VERSION}${RESET} ${SLATE_400}S:${SLATE_300}${skills_count}${RESET}\n"
        ;;
    micro)
        if [ -n "$session_display" ]; then
            ...
            printf "${SLATE_600}── │${RESET} ${AAI_P}A${AAI_A}A${AAI_I}I${RESET} ${AAI_A}STATUSLINE${RESET} ${SLATE_600}│ ${local_dashes}${RESET} ${AAI_SESSION}${session_display}${RESET}\n"
        else
            printf "${SLATE_600}── │${RESET} ${AAI_P}A${AAI_A}A${AAI_I}I${RESET} ${AAI_A}STATUSLINE${RESET} ${SLATE_600}│ ──────────────────${RESET}\n"
        fi
        printf "${AAI_LABEL}LOC:${RESET} ${AAI_CITY}${location_city}${RESET} ${SLATE_600}│${RESET} ${AAI_TIME}${current_time}${RESET} ${SLATE_600}│${RESET} ${AAI_WEATHER}${weather_str}${RESET}\n"
        printf "${SLATE_400}ENV:${RESET} ${SLATE_400}CC:${RESET} ${AAI_A}${cc_version}${RESET} ${SLATE_600}│${RESET} ${SLATE_500}AAI:${AAI_A}${AAI_VERSION}${RESET} ${SLATE_400}ALG:${AAI_A}${ALGO_VERSION}${RESET} ${SLATE_600}│${RESET} ${SLATE_400}S:${SLATE_300}${skills_count}${RESET} ${SLATE_400}W:${SLATE_300}${workflows_count}${RESET} ${SLATE_400}H:${SLATE_300}${hooks_count}${RESET}\n"
        ;;
    mini)
        if [ -n "$session_display" ]; then
            local_left="── │ AAI STATUSLINE │"
        ...
        printf "${AAI_LABEL}LOC:${RESET} ${AAI_CITY}${location_city}${RESET}${SLATE_600},${RESET} ${AAI_STATE}${location_state}${RESET} ${SLATE_600}│${RESET} ${AAI_TIME}${current_time}${RESET} ${SLATE_600}│${RESET} ${AAI_WEATHER}${weather_str}${RESET}\n"
        printf "${SLATE_400}ENV:${RESET} ${SLATE_400}CC:${RESET} ${AAI_A}${cc_version}${RESET} ${SLATE_600}│${RESET} ${SLATE_500}AAI:${AAI_A}${AAI_VERSION}${RESET} ${SLATE_400}ALG:${AAI_A}${ALGO_VERSION}${RESET} ${SLATE_600}│${RESET} ${WIELD_ACCENT}SK:${RESET}${SLATE_300}${skills_count}${RESET} ${WIELD_WORKFLOWS}WF:${RESET}${SLATE_300}${workflows_count}${RESET} ${WIELD_HOOKS}Hooks:${RESET}${SLATE_300}${hooks_count}${RESET}\n"
        ;;
    normal)
        if [ -n "$session_display" ]; then
            local_left="── │ AAI STATUSLINE │"
        ...
        printf "${AAI_LABEL}LOC:${RESET} ${AAI_CITY}${location_city}${RESET}${SLATE_600},${RESET} ${AAI_STATE}${location_state}${RESET} ${SLATE_600}│${RESET} ${AAI_TIME}${current_time}${RESET} ${SLATE_600}│${RESET} ${AAI_WEATHER}${weather_str}${RESET}\n"
        printf "${SLATE_400}ENV:${RESET} ${SLATE_400}CC:${RESET} ${AAI_A}${cc_version}${RESET} ${SLATE_600}│${RESET} ${SLATE_500}AAI:${AAI_A}${AAI_VERSION}${RESET} ${SLATE_400}ALG:${AAI_A}${ALGO_VERSION}${RESET} ${SLATE_600}│${RESET} ${WIELD_ACCENT}SK:${RESET} ${SLATE_300}${skills_count}${RESET} ${SLATE_600}│${RESET} ${WIELD_WORKFLOWS}WF:${RESET} ${SLATE_300}${workflows_count}${RESET} ${SLATE_600}│${RESET} ${WIELD_HOOKS}Hooks:${RESET} ${SLATE_300}${hooks_count}${RESET}\n"
        ;;
esac
...
LEARNING_CACHE="$AAI_DIR/MEMORY/STATE/learning-cache.sh"

/**
 * AAI Migration Merger
 *
 * Merges extracted content from old installations with new AAI system.
 */
...
/**
 * Merge extracted content into a target AAI installation
 */
  ...
  try {
    ...
    if (opts.migrateUserContent && content.userContent.files.length > 0) {
      const userResult = mergeDirectory(
        ...
        join(targetDir, 'skills', 'AAI', 'USER'),
        opts.onConflict
      );

/**
 * AAI Migration Extractor
 *
 * Extracts transferable content from existing AAI installations.
 */
...
export function extractContent(source: InstallationInfo): ExtractedContent {
  ...
  const userPath = join(source.path, 'skills', 'AAI', 'USER');
...
export function copyExtractedContent(
  ...
  if (opts.includeUserContent && content.userContent.files.length > 0) {
    const destPath = join(targetDir, 'skills', 'AAI', 'USER');

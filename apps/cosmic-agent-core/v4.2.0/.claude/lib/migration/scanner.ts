/**
 * AAI Migration Scanner
 *
 * Detects existing AAI installations and analyzes their content for migration.
 */
...
/**
 * Standard locations to check for AAI installations
 */
...
/**
 * Scan a directory to determine if it's a AAI installation
 */
export function scanInstallation(path: string): InstallationInfo {
  ...
  if (existsSync(skillsDir)) {
    ...
    try {
      ...
      // Check for AAI skill
      info.components.coreSkill = existsSync(join(skillsDir, 'AAI', 'SKILL.md'));

      // Check for USER content
      info.components.userContent = existsSync(join(skillsDir, 'AAI', 'USER'));
    } catch (e) {
...
/**
 * Find all AAI installations in standard locations
 */
...
export function formatInstallationInfo(info: InstallationInfo): string {
  ...
  lines.push(`    - AAI skill: ${info.components.coreSkill ? 'Yes' : 'No'}`);

/**
 * AAI Installation Validator
 *
 * Verifies that a AAI installation is complete and functional.
 */
...
/**
 * Run all validation checks on a AAI installation
 */
...
function validateStructure(path: string): ValidationCheck[] {
  ...
  const requiredDirs = [
    ...
    { path: 'skills/AAI', name: 'AAI skill' },
...
function validateSkills(path: string): ValidationCheck[] {
  ...
  try {
    ...
    // Check AAI skill has SKILL.md
    const coreSkillMd = join(skillsDir, 'AAI', 'SKILL.md');
    checks.push({
      name: 'AAI skill has SKILL.md',
      category: 'skills',
    ...
    const userContent = join(skillsDir, 'AAI', 'USER');
    checks.push({
...
export function quickValidate(path: string): { valid: boolean; errors: string[] } {
  ...
  if (!existsSync(join(path, 'skills', 'AAI', 'SKILL.md'))) {
    errors.push('AAI skill not found');
  }

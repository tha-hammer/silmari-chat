...
> Install [AAI (Agent Assistant Infrastructure)](https://github.com/hackerman-cosmic/Agent-Assistant-Infrastructure) with a single command.

## Quick Start

```bash
bash AAI-Install/install.sh
```
...
| # | Step | What It Does |
|---|------|-------------|
| 1 | **System Detection** | Detects OS, architecture, shell, installed tools (Bun, Git, Claude Code), timezone, and any existing AAI installation |
...
| 5 | **AAI Repository** | Clones the AAI repo to `~/.claude/` (or updates if already present) |
| 6 | **Configuration** | Generates `settings.json`, `.env`, directory structure, `aai` shell alias, and patches version files |
...
1. Collects or auto-discovers your ElevenLabs API key (checks `~/.config/AAI/.env`)
...
- No existing AAI → fresh install (vs. upgrade if detected)
...
- Port conflicts → configurable via `AAI_INSTALL_PORT` environment variable
...
AAI ships a complete `settings.json` template in the release repository. This template includes:
...
1. Clones the AAI repository (which includes the full `settings.json` template)
2. Merges only user-specific fields into the existing template:
   ...
   - `env` — AAI_DIR, PROJECTS_DIR
   - `pai` — version info
3. Preserves all hooks, status line, spinner verbs, and context files from the template

This ensures fresh installs get the full AAI configuration without the installer needing to know about every hook.
...
| `.env` | `~/.config/AAI/.env` | `ELEVENLABS_API_KEY=...` |
| `LATEST` | `~/.claude/AAI/Algorithm/LATEST` | Algorithm version (patched to current) |
| Shell alias | `~/.zshrc` | `alias aai='bun ~/.claude/AAI/Tools/pai.ts'` |
...
On first launch after installation, the AAI banner displays system statistics (skills, hooks, workflows, signals, files). These counts are:
...
The Algorithm version displayed in the banner reads from `AAI/Algorithm/LATEST`.
...
- **Dark theme** — Deep navy/black with AAI blue accents and glassmorphic card effects
...
This reloads your shell config (activates the `aai` alias) and launches AAI for the first time.
...
| Port 1337 in use | Set `AAI_INSTALL_PORT=8080` before running install.sh |
...
| `aai` command not found | Run `source ~/.zshrc` to reload shell config |
| Voice server won't start | Check port 8888 is free: `lsof -ti:8888`. Kill any process using it. |
| Banner shows wrong algorithm version | Check `~/.claude/AAI/Algorithm/LATEST` contains correct version |
...
bun run AAI-Install/main.ts --mode web

# CLI mode
bun run AAI-Install/main.ts --mode cli

# GUI mode (Electron — installs deps on first run)
bun run AAI-Install/main.ts --mode gui
```
...
Part of [AAI — Agent Assistant Infrastructure](https://github.com/hackerman-cosmic/Agent-Assistant-Infrastructure).

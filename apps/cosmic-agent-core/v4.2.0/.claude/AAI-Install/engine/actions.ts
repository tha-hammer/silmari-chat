/**
 * AAI Installer v4.0 — Install Actions
...
import { AAI_VERSION, ALGORITHM_VERSION } from "./types";
 ...
 * Fixes: https://github.com/cosmic-hq/Personal_AI_Infrastructure/issues/954
 */
...
function findExistingEnvKey(keyName: string): string {
  ...
  // Check ~/.config/AAI/.env
  searchPaths.push(join(home, ".config", "AAI", ".env"));
  ...
  try {
    ...
    for (const entry of homeEntries) {
      if (entry.startsWith(".claude") && entry !== ".claude") {
        ...
        searchPaths.push(join(home, entry, ".config", "AAI", ".env"));
      }
    }
  } catch {
...
//
// In v2.5–v3.0, user context (ABOUTME.md, LifeOS/, CONTACTS.md, etc.)
// lived at skills/AAI/USER/ (or skills/CORE/USER/ in v2.4).
// In v4.0, user context moved to AAI/USER/ and CONTEXT_ROUTING.md
...
/**
 * Migrate user context from legacy skills/AAI/USER or skills/CORE/USER
 * to the canonical AAI/USER location. Replaces the legacy directory
...
async function migrateUserContext(
  ...
  const newUserDir = join(paiDir, "AAI", "USER");
  if (!existsSync(newUserDir)) return; // AAI/USER/ not set up yet

  const legacyPaths = [
    join(paiDir, "skills", "AAI", "USER"),   // v2.5–v3.0
  ...
  for (const legacyDir of legacyPaths) {
    ...
    const label = legacyDir.includes("CORE") ? "skills/CORE/USER" : "skills/AAI/USER";
    ...
    if (copied > 0) {
      await emit({ event: "message", content: `Migrated ${copied} user context files from ${label} to AAI/USER.` });
    }

    // Replace legacy dir with symlink so skill-relative paths still work
    try {
      rmSync(legacyDir, { recursive: true });
      // Symlink target is relative: from skills/AAI/ or skills/CORE/ → ../../AAI/USER
      symlinkSync(join("..", "..", "AAI", "USER"), legacyDir);
      await emit({ event: "message", content: `Replaced ${label} with symlink to AAI/USER.` });
    } catch {
...
export async function runSystemDetect(
  ...
  if (detection.existing.paiInstalled) {
    ...
    await emit({
      event: "message",
      content: `Existing AAI installation detected (v${detection.existing.paiVersion || "unknown"}). This will upgrade your installation.`,
    });
  } else {
    state.installType = "fresh";
    await emit({ event: "message", content: "No existing AAI installation found. Starting fresh install." });
  }
...
export async function runPrerequisites(
  ...
  // Fixes: https://github.com/cosmic-hq/Personal_AI_Infrastructure/issues/856
...
export async function runIdentity(
  ...
  state.collected.aiName = aiName.trim() || defaultAi || "AAI";
...
export async function runRepository(
  ...
  if (state.installType === "upgrade") {
    ...
    if (isGitRepo) {
      ...
      if (pullResult !== null) {
        await emit({ event: "message", content: "AAI repository updated from GitHub." });
      } else {
    ...
    await emit({ event: "progress", step: "repository", percent: 20, detail: "Cloning AAI repository..." });
    ...
    const cloneResult = tryExec(
      `git clone https://github.com/hackerman-cosmic/Agent-Assistant-Infrastructure.git "${paiDir}" 2>&1`,
      120000
    );

    if (cloneResult !== null) {
      await emit({ event: "message", content: "AAI repository cloned successfully." });
    } else {
      ...
      const initResult = tryExec(`cd "${paiDir}" && git init && git remote add origin https://github.com/hackerman-cosmic/Agent-Assistant-Infrastructure.git && git fetch origin && git checkout -b main origin/main 2>&1`, 120000);
      if (initResult !== null) {
        await emit({ event: "message", content: "AAI repository initialized and synced." });
      } else {
        await emit({
          event: "message",
          content: "Could not clone AAI repo automatically. You can clone it manually later: git clone https://github.com/hackerman-cosmic/Agent-Assistant-Infrastructure.git ~/.claude",
        });
      }
    }
  }
...
export async function runConfiguration(
  ...
  if (existsSync(settingsPath)) {
    try {
      ...
      existing.env = normalizeSettingsEnv(existing.env, config.env);
      ...
      existing.pai.version = AAI_VERSION;
  ...
  const latestPath = join(paiDir, "AAI", "Algorithm", "LATEST");
  const latestDir = join(paiDir, "AAI", "Algorithm");
  ...
  try {
    ...
    const fileCount = countFiles(join(paiDir, "skills", "AAI", "USER"));
  ...
  const aliasLine = `alias aai='bun ${join(paiDir, "AAI", "Tools", "pai.ts")}'`;
  const marker = "# AAI alias";

  if (existsSync(rcPath)) {
    let content = readFileSync(rcPath, "utf-8");
    // Remove any existing pai/aai alias (old CORE or AAI paths, any marker variant)
    content = content.replace(/^#\s*(?:AAI|CORE)\s*alias.*\n.*alias (?:pai|aai)=.*\n?/gm, "");
    content = content.replace(/^alias (?:pai|aai)=.*\n?/gm, "");
    // Add fresh alias
...
export async function runVoiceSetup(
  ...
  if (!hasElevenLabsKey) {
    await emit({ event: "message", content: "No ElevenLabs key — voice server will use macOS text-to-speech as fallback. You can add a key later in ~/.config/AAI/.env" });
  }
  ...
  if (voiceServerReady) {
    ...
    while (!voiceConfirmed) {
      ...
      try {
        const aiName = state.collected.aiName || "AAI";

/**
 * AAI Installer v4.0 — System Detection
 * Detects OS, tools, existing AAI installation, and environment.
...
function detectExisting(
  ...
  // Check for existing AAI installation
  ...
  // Check for existing AAI skill
  if (existsSync(join(paiDir, "skills", "AAI", "SKILL.md"))) {
    result.paiInstalled = true;
  }
...
export function detectSystem(): DetectionResult {
  ...
  const configDir = process.env.AAI_CONFIG_DIR || join(home, ".config", "AAI");

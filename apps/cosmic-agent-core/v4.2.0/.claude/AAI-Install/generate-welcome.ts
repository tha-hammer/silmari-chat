#!/usr/bin/env bun
/**
 * AAI Installer v4.0 — Welcome MP3 Generator
...
async function generateWelcome() {
  ...
  if (!apiKey) {
    ...
    const envPath = join(homedir(), ".config", "AAI", ".env");
    ...
    if (!process.env.ELEVENLABS_API_KEY) {
      console.error("Error: ELEVENLABS_API_KEY not found in environment or ~/.config/AAI/.env");
  ...
  const text = "Welcome to Agent Assistant Infrastructure. <break time=\"1.0s\" /> Maximizing human potential.";

/**
 * AAI Installer v4.0 — CLI Display Helpers
...
export function printBanner(): void {
  ...
  print("");
  print(`                      ${c.navy}P${c.reset}${c.blue}A${c.reset}${c.lightBlue}I${c.reset} ${c.steel}|${c.reset} ${c.gray}Agent Assistant Infrastructure${c.reset}`);
  print("");
  print(`                     ${c.italic}${c.lightBlue}"Maximizing human potential..."${c.reset}`);
  print("");
  print("");
  ...
  print(`           ${c.navy}████${c.reset}        ${c.navy}████${c.reset}${c.lightBlue}████${c.reset}   ${sep}  ${c.navy}⬢${c.reset}  ${c.gray}AAI v4.0.3${c.reset}`);
  ...
  print("");
  print("");
  print(`                       ${c.steel}→${c.reset} ${c.blue}github.com/hackerman-cosmic/Agent-Assistant-Infrastructure${c.reset}`);
  print("");
  print(`${c.steel}┗━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━┛${c.reset}`);
  print("");
}
...
export function printDetection(det: DetectionResult): void {
  ...
  if (det.existing.paiInstalled) {
    printInfo(`Existing AAI: v${det.existing.paiVersion} (upgrade mode)`);
  } else {
    printInfo("Existing AAI: not detected (fresh install)");
  }
...
export function printSummary(summary: InstallSummary): void {
  ...
  print(`${c.navy}╠══════════════════════════════════════════════════╣${c.reset}`);
  print(`${c.navy}║${c.reset}  AAI Version:  ${c.white}v${summary.paiVersion}${c.reset}                             ${c.navy}║${c.reset}`);
  ...
  print(`${c.navy}║${c.reset}  ${c.lightBlue}Run: ${c.bold}source ~/.zshrc && aai${c.reset}                      ${c.navy}║${c.reset}`);
  print(`${c.navy}║${c.reset}                                                  ${c.navy}║${c.reset}`);
  print(`${c.navy}╚══════════════════════════════════════════════════╝${c.reset}`);
  print("");
}

...
process.on("uncaughtException", (err) => {
  console.error("[AAI Installer] Uncaught exception:", err.message);
});
process.on("unhandledRejection", (err: any) => {
  console.error("[AAI Installer] Unhandled rejection:", err?.message || err);
});
...
const PORT = parseInt(process.env.AAI_INSTALL_PORT || "1337");
...
function resetInactivity(): void {
  ...
  inactivityTimer = setTimeout(() => {
    console.log("\n[AAI Installer] Shutting down due to inactivity.");
    process.exit(0);
  }, INACTIVITY_MS);
}
...
console.log(`AAI Installer server running on http://127.0.0.1:${PORT}/`);

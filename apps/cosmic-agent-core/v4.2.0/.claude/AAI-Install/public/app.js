/**
 * AAI Installer v4.0 — Frontend Application
...
let steps = [
  ...
  { id: 'repository', name: 'AAI Repository', number: 5, status: 'pending' },
...
function renderDetection(data) {
  ...
  const items = [
    ...
    { icon: data.existing?.paiInstalled ? 'info' : 'check', label: 'Existing AAI', value: data.existing?.paiInstalled ? 'v' + (data.existing.paiVersion || '?') : 'Fresh install' },
...
function renderSummary(summary) {
  ...
  card.innerHTML = `
    ...
    <div class="summary-row"><span class="s-label">AAI Version</span><span class="s-value">v${summary.paiVersion}</span></div>
    ...
    <div class="summary-action">
      <p>To activate AAI, open a terminal and run:</p>
      <code>source ~/.zshrc && aai</code>
      <p class="summary-hint">This reloads your shell config and launches AAI for the first time.</p>
    </div>
  `;

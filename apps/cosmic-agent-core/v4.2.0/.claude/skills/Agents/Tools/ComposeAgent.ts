 ...
 * Merges base traits (ships with AAI) with user customizations.
 *
 * Configuration files:
 *   Base:  ~/.claude/skills/Agents/Data/Traits.yaml
 *   User:  ~/.claude/AAI-USER/SKILLCUSTOMIZATIONS/Agents/Traits.yaml
 *
 * Usage:
...
const USER_TRAITS_PATH = `${HOME}/.claude/AAI/USER/SKILLCUSTOMIZATIONS/Agents/Traits.yaml`;
...
**USE THE AAI FORMAT FROM AAI FOR ALL RESPONSES:**
...
- Use AAI output format for all responses
...
CONFIGURATION:
  ...
  User traits:    ~/.claude/AAI-USER/SKILLCUSTOMIZATIONS/Agents/Traits.yaml

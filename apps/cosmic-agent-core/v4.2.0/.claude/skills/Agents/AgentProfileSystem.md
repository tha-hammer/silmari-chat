...
2. Which parts of the AAI Skills system are relevant to their work
...
- ❌ Duplicate content from AAI (AAI auto-loads at session start)
...
- ✅ Supplement what AAI already provides
...
## Key Principles (from AAI)

[Brief list - these are ALREADY LOADED via AAI, just reference them]
...
The agent receives:
1. **AAI context** (auto-loaded at session start)
...
1. **No Duplication**: AAI already loads constitutional principles, stack preferences, etc. No need to repeat them.
...
4. **Supplements, Doesn't Replace**: Adds to what AAI provides, doesn't try to replace it.
...
2. **Project-Specific Context**: Load `.sai/agent-context.md` for project-specific patterns
...
- Had memory blocks that duplicated AAI content

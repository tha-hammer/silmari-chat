...
`~/.claude/AAI-USER/SKILLCUSTOMIZATIONS/BrightData/`
...
→ **EXECUTE:** Four-tier progressive scraping workflow (WebFetch → Curl → Markdown Web Browser → Bright Data MCP)
...
3. **Tier 3: Markdown Web Browser** - Stealth Chrome rendering + OCR → clean markdown (~/Dev/markdown_web_browser)
...
- Tier 3 uses stealth Chrome with 60+ anti-detection measures and OCR for image-heavy content
...
- **Process:** Start with WebFetch → If fails, use curl with Chrome headers → If fails, use Markdown Web Browser (stealth Chrome + OCR) → If fails, use Bright Data MCP
...
- **Markdown Web Browser** - Stealth Chrome + OCR → markdown (~/Dev/markdown_web_browser, localhost:8000)
...
- **Tier 3 (Markdown Web Browser):** Sites requiring JavaScript execution, Cloudflare-protected sites, dynamic content, image-heavy pages needing OCR
...
**Example 2: Site with JavaScript/Cloudflare (Tier 3 — Markdown Web Browser)**

User: "Can't access this site https://finviz.com"

Skill Response:
1. Routes to four-tier-scrape.md
2. Attempts Tier 1 (WebFetch) → Fails (Cloudflare block)
3. Attempts Tier 2 (Curl with Chrome headers) → Fails (JS challenge)
4. Attempts Tier 3 (Markdown Web Browser) → Success (stealth Chrome + OCR)
5. Returns clean markdown with extracted data from charts/tables
6. Total time: ~20-30 seconds

**Example 3: Site with Advanced Bot Detection (Tier 4)**
...
4. Attempts Tier 3 (Markdown Web Browser) → Fails (CAPTCHA)
...
- `~/.claude/AAI/SkillSystem.md` - Canonical structure guide
- `~/.claude/AAI/CONSTITUTION.md` - Overall AAI philosophy

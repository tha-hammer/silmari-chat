...
**Purpose:** Generate a McKinsey-style professional consulting report from LifeOS analysis content, rendered as a dark-themed web-based document with custom Practical Typography fonts and automatic light-mode print support.
...
│  Run LifeOS         Generate story     Map to McKinsey   Output as  │
...
|-----------|----------|---------|-------------|
| `source` | Yes | - | LifeOS directory or analysis context |
...
cp -r ~/.claude/skills/LifeOS/ReportTemplate/* {output_dir}/
...
- **Centered content block:**
  ...
  - **"LifeOS Assessment"** label (Heliotrope Caps, primary blue, tracking-[0.25em])
...
```bash
# User: "Create a LifeOS report for Acme Corp"

# Step 1: {DAIDENTITY.NAME} runs LifeOS analysis on source directory
...
```
~/.claude/skills/LifeOS/ReportTemplate/
```
...
2. Generate `lib/report-data.ts` with client-specific content:
   ...
   - All findings, recommendations, roadmap from LifeOS analysis
...
- Direct LifeOS directory analysis
...
- [ ] "LifeOS Assessment" label visible above title
...
Edit files in `~/.claude/skills/LifeOS/ReportTemplate/components/`

**To change color scheme:**
Edit CSS custom properties in `~/.claude/skills/LifeOS/ReportTemplate/app/globals.css`

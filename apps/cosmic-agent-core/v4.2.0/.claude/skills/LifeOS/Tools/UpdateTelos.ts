#!/usr/bin/env bun
/**
 * update-LifeOS - Update LifeOS life context with automatic backups and change tracking
 *
 * This command manages updates to the LifeOS life context files, ensuring:
 * - Automatic timestamped backups before any modification
 * - Change tracking in updates.md
 * - Complete version history
 *
 * Usage:
 *   update-LifeOS <file> "<content>" "<change-description>"
 *
 * Example:
 *   update-LifeOS BOOKS.md "- Project Hail Mary by Andy Weir" "Added new favorite book"
 *
...
const LifeOS_DIR = join(process.env.HOME!, '.claude', 'AAI', 'USER', 'LifeOS');
const BACKUPS_DIR = join(LifeOS_DIR, 'Backups');
const UPDATES_FILE = join(LifeOS_DIR, 'Updates.md');
...
const VALID_FILES = [
  ...
  'LifeOS.md', 'TRAUMAS.md', 'WISDOM.md', 'WRONG.md'
];
...
async function main() {
  ...
  if (args.length < 3) {
    console.error('❌ Usage: update-LifeOS <file> "<content>" "<change-description>"');
    console.error('\nExample: update-LifeOS BOOKS.md "- New Book Title" "Added favorite book"');
  ...
  const targetFile = join(LifeOS_DIR, filename);

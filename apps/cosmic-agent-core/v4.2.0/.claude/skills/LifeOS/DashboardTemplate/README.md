...
2. **Ask** (`/ask`) - AI chatbot powered by Claude Haiku 4.5 with full LifeOS context
3. **Add File** (`/add-file`) - Drag-and-drop file upload interface for .md and .csv files
4. **File Viewer** (`/file/[slug]`) - Dynamic file viewer with edit capabilities for all LifeOS files
...
```bash
cp -r ~/.claude/skills/LifeOS/dashboard-template /path/to/your/project
...
The template includes a complete file management system for LifeOS files (markdown and CSV):

### File Discovery
- **Automatic scanning**: The dashboard automatically discovers all `.md` and `.csv` files in the LifeOS directory
...
**Implementation**: `lib/LifeOS-data.ts`
- `getAllLifeOSData()` - Scans LifeOS directory and returns all files
- `getLifeOSFileCount()` - Returns total file count
- `getLifeOSFileList()` - Returns array of filenames
- Searches `~/.claude/skills/LifeOS/` for `.md` files
- Searches `~/.claude/skills/LifeOS/data/` for `.csv` files
...
- Saves to appropriate LifeOS directory
- Logs uploads to `updates.md`

### File Viewing and Editing
**Page**: `app/file/[slug]/page.tsx`
- Dynamic route for any LifeOS file
...
window.dispatchEvent(new CustomEvent('LifeOSFileUploaded', {
  detail: { filename: 'example.md' }
}))

// Listened to by sidebar for automatic refresh
window.addEventListener('LifeOSFileUploaded', handleFileUploaded)
```
...
│   ├── LifeOS-data.ts             # LifeOS file system utilities
...
This template is part of the LifeOS skill system and is free to use for any purpose.

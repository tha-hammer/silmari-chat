...
function loadConfig(): Config {
  const envPath = process.env.PAI_CONFIG_DIR ? join(process.env.PAI_CONFIG_DIR, '.env') : join(homedir(), '.config', 'AAI', '.env');
...
**Reference:** `~/.claude/AAI/CliFirstArchitecture.md` (Configuration Flags section)

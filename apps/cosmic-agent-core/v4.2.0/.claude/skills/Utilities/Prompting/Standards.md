...
## Native Fabric Patterns in AAI

**Location:** `~/.claude/skills/Utilities/Fabric/Patterns/`

AAI maintains a local copy of all Fabric patterns for native execution. Instead of spawning the `fabric` CLI for every pattern-based task, the system reads and applies patterns directly as prompts.
...
This pulls upstream updates via `fabric -U` and syncs to AAI's local copy.
...
**Foundation:** Based on Anthropic's official `{{variable}}` syntax, industry patterns (LangChain, Handlebars, DSPy), and AAI's unique skill architecture.

**Philosophy:** Templates enable prompts to write prompts—dynamic composition where structure is fixed but content is parameterized. This is core AAI DNA.

---

## Core Syntax

AAI uses Handlebars notation for template variables:

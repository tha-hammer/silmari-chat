 ...
 * AI-powered recommendations for improving AAI infrastructure.
 *
 * Usage:
...
async function fetchGitHubRepo(source: Source, state: State): Promise<Update[]> {
  ...
  const headers: Record<string, string> = {
    ...
    'User-Agent': 'AAI-Anthropic-Monitor'
  };
...
// Recommendation engine - AAI AAI ecosystem focused
function generateRecommendation(update: Update): string {
  const { source, type, title, category } = update;
  const titleLower = title.toLowerCase();

  // SKILLS - Critical for AAI's skill system
  if (titleLower.includes('skill') || titleLower.includes('skills')) {
    return `**AAI Impact:** CRITICAL for skills ecosystem\n` +
      `**Why:** AAI's entire infrastructure is built on skills - any changes to skill patterns, specifications, or examples directly affect how we build and organize AAI's capabilities.\n` +
      `**Action:** Review immediately and update AAI's skill templates/patterns if new conventions emerge. Check if new skill categories or capabilities can be adopted.`;
  }

  // MCP - Core infrastructure
  if (titleLower.includes('mcp') || source.toLowerCase().includes('mcp')) {
    return `**AAI Impact:** HIGH - MCP infrastructure enhancement\n` +
      `**Why:** AAI uses MCP servers for chrome-devtools, brightdata, Ref docs, content access, and Stripe. Changes to MCP spec/docs affect our integrations.\n` +
      `**Action:** Assess compatibility with existing MCP servers in .mcp.json. Look for new MCP capabilities to expand AAI's tooling.`;
  }

  // Commands/Slash Commands
  if (titleLower.includes('command') || titleLower.includes('slash command')) {
    return `**AAI Impact:** HIGH - Command system update\n` +
      `**Why:** AAI uses slash commands extensively (~/.claude/Commands/). Changes affect our command architecture and user workflows.\n` +
      `**Action:** Review for new command patterns or capabilities. Update AAI's command templates if conventions change.`;
  }

  // Agents/Hooks
  if (titleLower.includes('agent') || titleLower.includes('hook')) {
    return `**AAI Impact:** HIGH - Agent/Hook system change\n` +
      `**Why:** AAI uses agents (researcher, engineer, architect, etc.) and hooks (load-context, stop-hook) as core infrastructure components.\n` +
      `**Action:** Check if this affects AAI's agent definitions or hook configurations. Test existing agent workflows.`;
  }

  // Claude Code releases
  if (type === 'release' && source.includes('claude-code')) {
    return `**AAI Impact:** CRITICAL - Core platform update\n` +
      `**Why:** AAI runs on Claude Code - releases may include new features, breaking changes, or performance improvements.\n` +
      `**Action:** Review changelog carefully. Test AAI's critical workflows. Update skills/commands if APIs changed.`;
  }

  // MCP releases
  if (type === 'release' && source.includes('MCP')) {
    return `**AAI Impact:** HIGH - MCP protocol update\n` +
  ...
  if (titleLower.includes('plugin') || titleLower.includes('marketplace')) {
    return `**AAI Impact:** MEDIUM - Ecosystem expansion\n` +
      `**Why:** Plugin/marketplace features could provide new capabilities to integrate into AAI's toolkit.\n` +
      `**Action:** Explore available plugins. Assess if any solve current AAI limitations or add valuable features.`;
  }
  ...
  if (source.includes('cookbook') || source.includes('quickstart') || source.includes('courses')) {
    return `**AAI Impact:** MEDIUM - Implementation patterns\n` +
      `**Why:** Cookbooks/examples show best practices and patterns we can adopt in AAI's codebase.\n` +
      `**Action:** Review for reusable patterns, especially around skills, agents, or Claude Code features. Extract learnings for AAI.`;
  }

  // GitHub commits
  if (category === 'github' && type === 'commit') {
    return `**AAI Impact:** LOW-MEDIUM - Code pattern review\n` +
      `**Why:** Commits may reveal implementation details, bug fixes, or patterns useful for AAI development.\n` +
  ...
  if (titleLower.includes('doc') || type === 'docs') {
    return `**AAI Impact:** MEDIUM - Capability discovery\n` +
      `**Why:** Doc updates often reveal new features or best practices not yet in AAI.\n` +
      `**Action:** Review for new Claude Code features, API capabilities, or configuration options to leverage.`;
  }

  // SDK releases
  if (source.includes('sdk')) {
    return `**AAI Impact:** LOW - SDK update\n` +
      `**Why:** SDK updates are less relevant since AAI uses Claude Code CLI, not raw API SDKs.\n` +
      `**Action:** Note for reference. Only investigate if mentions features relevant to AAI's agent implementations.`;
  }

  // Blog posts
  if (type === 'blog') {
    return `**AAI Impact:** LOW-MEDIUM - Awareness\n` +
      `**Why:** Blogs announce new features and directions that may eventually affect AAI.\n` +
      `**Action:** Skim for strategic announcements about Claude Code, Skills, or MCP. Track for future planning.`;
  }

  // Generic
  return `**AAI Impact:** LOW - General awareness\n` +
    `**Why:** May have indirect relevance to AAI ecosystem.\n` +
    `**Action:** Review if time permits. Low impact on AAI's core functionality.`;
}
...
// Generate narrative analysis focused on AAI ecosystem
function generateNarrative(updates: Update[]): string {
  ...
  let narrative = `## 📖 Executive Summary: What This Means for AAI\n\n`;

  // Overall activity
  narrative += `Found **${updates.length} updates** across the Anthropic ecosystem in the monitored period. `;

  if (high.length > 0) {
    narrative += `**${high.length} are HIGH priority** for AAI's infrastructure, `;
  }
  if (medium.length > 0) {
  ...
  if (skillUpdates.length > 0) {
    ...
    if (highSkills > 0) {
      ...
      narrative += `There are **${highSkills} HIGH-priority skill updates** - this is BIG because AAI's entire architecture is built on the skills system. `;
      narrative += `Any changes to skill patterns, specifications, or conventions could require updates to AAI's ${countSkills()} existing skills. `;
      ...
      narrative += `\n\n**→ Priority Action:** Review all skill updates immediately. Update AAI's skill templates if conventions changed.\n\n`;
    } else {
  ...
  if (mcpUpdates.length > 0) {
    ...
    if (highMcp > 0) {
      ...
      narrative += `**${highMcp} HIGH-priority MCP updates** detected. Since AAI uses MCP servers for chrome-devtools, brightdata, Ref, content, and Stripe, `;
      ...
      narrative += `\n\n**→ Priority Action:** Test existing MCP servers. Look for new MCP features to expand AAI's toolkit.\n\n`;
    } else {
  ...
  if (codeUpdates.length > 0) {
    ...
    if (highCode > 0) {
      ...
      narrative += `**${highCode} HIGH-priority** Claude Code updates found. Since AAI runs on Claude Code, platform changes can affect everything. `;
      ...
      narrative += `\n\n**→ Priority Action:** Review changelog. Test AAI's core workflows after updating.\n\n`;
    } else {
  ...
  if (cookbookUpdates.length > 0) {
    ...
    if (skillCookbooks.length > 0) {
      narrative += `${skillCookbooks.length} specifically about skills - definitely review these for patterns to adopt in AAI. `;
    }
    narrative += `\n\n`;
  }
  ...
  if (docUpdates.length > 5) {
    ...
    narrative += `**${docUpdates.length} documentation pages** updated. While less urgent, docs often reveal new capabilities or best practices not yet used in AAI.\n\n`;
  }
  ...
  if (high.length > 5) {
    ...
    narrative += `**Quiet period** - mostly low-priority updates. Good time to focus on AAI development rather than external changes.\n\n`;
  }
  ...
  if (skillUpdates.filter(u => u.priority === 'HIGH').length > 0) {
    topItems.push('1. **Skills system updates** - Review immediately, may affect AAI architecture');
  }
...
function countSkills(): number {
  ...
  return 20; // Approximate based on AAI's current skill set
}

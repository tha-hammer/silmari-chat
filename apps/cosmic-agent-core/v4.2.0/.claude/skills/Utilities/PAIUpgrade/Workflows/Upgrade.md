...
**Trigger:** "check for upgrades", "upgrade", "any updates", "check Anthropic", "check YouTube", "sai upgrade"
...
1. **Thread 1:** Analyze user context (PRAXOS, projects, recent work, AAI state)
...
"Read and analyze the user's PRAXOS files to understand their current focus:
...
"Analyze the current state of the user's AAI system:
...
Return format for EACH technique:
{
  ...
  'current_pai_gap': '[What AAI currently does/lacks that this addresses]',
  'implementation_file': '[Path to AAI file this would change]',
...
1. Load channel config:
   bun ~/.claude/AAI/Tools/LoadSkillConfig.ts ../youtube-channels.json
...
4. For NEW videos, extract transcripts:
   bun ~/.claude/AAI/Tools/GetTranscript.ts '<video-url>'
...
5. CRITICAL - For each transcript, extract SPECIFIC TECHNIQUES:
   ...
   - Identify what AAI component this applies to
...
Return format for EACH technique found:
{
  ...
  'pai_relevance': '[Which AAI component this could improve]'
}
...
1. Look in ~/.claude/AAI-USER/SKILLCUSTOMIZATIONS/AAIUpgrade/
...
"Discover trending GitHub projects relevant to AAI for inspiration.

1. Load the github_trending config from user-sources.json:
   Read ~/.claude/AAI-USER/SKILLCUSTOMIZATIONS/AAIUpgrade/user-sources.json
...
6. For EACH new repo found, evaluate AAI relevance:
   - Read the repo's README (first 500 lines): gh api 'repos/OWNER/REPO/readme' --jq '.content' | base64 -d | head -500
   - Assess: Does this repo solve a problem AAI has? Does it use patterns AAI could adopt?
...
7. Return format for EACH relevant project:
   {
     ...
     'topics': ['[topics]'],
     'created': '[date]',
     'pai_relevance': '[1-2 sentences: what problem this solves that AAI cares about]',
     'inspiration_techniques': [
       {
         'technique': '[specific pattern, architecture, or approach worth noting]',
         'applies_to': '[which AAI component could benefit]'
       }
     ]
   }
...
- Connect findings to specific AAI components that could benefit.
...
- **🔴 CRITICAL:** Score > 30, relevance > 8. Fixes gaps, security issues, or unlocks capabilities AAI should already have. Integrate immediately.
- **🟠 HIGH:** Score 22-30, relevance > 6. Significantly improves AAI capabilities or efficiency. Integrate this week.
...
- AAI Relevance (WHY it matters for our system — this is the primary framing, not an afterthought)
- Effort estimate (Low/Med/High)
- Files affected (specific AAI files that would change)
...
1. **✨ Discoveries** — Everything interesting found, ranked by coolness/interestingness, with source and AAI relevance. This is the "what's out there" overview {PRINCIPAL.NAME} reads first.
...
|---|-----------|--------|---------------------|---------------|
| 1 | [Most interesting thing] | [source] | [What makes this cool — 1-2 sentences] | [How it maps to AAI] |
| 2 | [Next most interesting] | [source] | [Why it's notable] | [AAI connection] |
| ... | ... | ... | ... | ... |
...
[Fixes gaps, security issues, or unlocks capabilities AAI should already have]

| # | Recommendation | AAI Relevance | Effort | Files Affected |
|---|---------------|---------------|--------|----------------|
| [N] | [Short action] | [Why AAI needs this NOW] | [Low/Med/High] | `[files]` |

### 🟠 HIGH — Integrate this week
[Significantly improves AAI capabilities or efficiency]
...
[Describe the specific AAI benefit. Must be 16-32 words.]
...
**How It Helps AAI (16-32 words):**
[Describe which AAI component could adopt this pattern and what improvement it would bring.]

**Inspiration Techniques:**
> [Specific architectural pattern, approach, or code technique worth noting from the repo's README or code]

**Applies To:** `[AAI file path]`, [component name]
**Potential Integration:**
[How AAI could adopt this pattern — not a full implementation, but the key insight to borrow]
...
2. **DISCOVERIES ≠ RECOMMENDATIONS** - The Discoveries table ranks by "how cool is this" (interestingness). The Recommendations section ranks by "how urgently should AAI integrate this" (priority). These are DIFFERENT orderings. A LOW-priority item can be Discovery #1 if it's the most interesting thing found.
3. **RECOMMENDATIONS COME SECOND** - The 🔥 Recommendations section with four priority tiers follows the Discoveries table.
4. **AAI RELEVANCE IS PRIMARY** - In BOTH the Discoveries table AND the Recommendation tables, explain WHY this matters for AAI.
...
8. **MAP TO AAI** - Every technique must name a specific AAI file or component it improves
9. **TWO MANDATORY DESCRIPTIONS (16-32 words each):**
   ...
   - **How It Helps AAI:** Describe the specific benefit - which component improves, what gap it fills
...
**How It Helps AAI (16-32 words):**
Our session documentation workflows had manual session ID extraction hacks. Native substitution means cleaner code and reliable session tracking across all AAI workflows.
...
| SDK update v0.78 | GitHub | AAI uses CLI, not raw SDK |

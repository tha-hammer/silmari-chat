...
This workflow helps identify new sources worth monitoring for AAI-relevant updates:
...
- Content relevance to AAI infrastructure
...
**URL:** [url]
**Relevance:** [Why this matters for AAI]
...
Edit `~/.claude/AAI-USER/SKILLCUSTOMIZATIONS/AAIUpgrade/youtube-channels.json`

### For Other Sources:
Currently, non-YouTube sources are monitored via the base `sources.json`.
To request additions to base Anthropic monitoring, note them for next AAI release.
```
...
cat ~/.claude/AAI-USER/SKILLCUSTOMIZATIONS/AAIUpgrade/youtube-channels.json

...
**API Key:** Stored in `~/.config/AAI/.env` as `REMOVEBG_API_KEY`
...
The remove.bg API key is stored in `~/.config/AAI/.env`:
...
**Solution:** Verify `REMOVEBG_API_KEY` is set correctly in `~/.config/AAI/.env`

# AAI Pack Icon Workflow

**Generate 256x256 transparent PNG icons for AAI packs.**
...
Create consistent, professional icons for AAI packs following the established visual identity.
...
3. **Consistent style** - Match existing AAI pack icons
...
**Good icon concepts:**
- `sai-hook-system` → Hook shape, event trigger
- `sai-core-install` → Download/install arrow
- `sai-skill-system` → Brain/routing/capability
- `sai-agent-system` → Robot/assistant figure
- `sai-voice-system` → Sound wave/speaker
...
bun run ~/.claude/skills/Media/Art/Tools/Generate.ts \
  ...
  --aspect-ratio 1:1 \
  --remove-bg \
  --output ${PROJECTS_DIR}/AAI/Packs/icons/[PACK_NAME].png
```
...
ls -la ${PROJECTS_DIR}/AAI/Packs/icons/[PACK_NAME].png

# Check dimensions (requires imagemagick)
file ${PROJECTS_DIR}/AAI/Packs/icons/[PACK_NAME].png
```
...
bun run ~/.claude/skills/Media/Art/Tools/Generate.ts \
  ...
  --aspect-ratio 1:1 \
  --remove-bg \
  --output ${PROJECTS_DIR}/AAI/Packs/icons/AAI-hook-system.png
```
...
bun run ~/.claude/skills/Media/Art/Tools/Generate.ts \
  ...
  --aspect-ratio 1:1 \
  --remove-bg \
  --output ${PROJECTS_DIR}/AAI/Packs/icons/AAI-core-install.png
```
...
bun run ~/.claude/skills/Media/Art/Tools/Generate.ts \
  ...
  --aspect-ratio 1:1 \
  --remove-bg \
  --output ${PROJECTS_DIR}/AAI/Packs/icons/AAI-memory-system.png
```
...
All AAI pack icons go to:
```
${PROJECTS_DIR}/AAI/Packs/icons/[PACK_NAME].png
```

**Naming convention:** Match the pack directory name exactly.
- Pack: `Packs/AAI-hook-system/`
- Icon: `Packs/icons/AAI-hook-system.png`
...
- [ ] **Exists** at `${PROJECTS_DIR}/AAI/Packs/icons/[PACK_NAME].png`
...
- [ ] **Consistent** - matches other AAI icons in style

---

## Related Workflows

- `the AAI release workflow` - Release workflow (may include icon generation)

...
**The foundational pattern for AAI extensibility and personalization**

---

## Overview

AAI uses a consistent two-tier architecture across all configurable components:

```
SYSTEM tier  →  Base functionality, defaults, AAI updates
USER tier    →  Personal customizations, private policies, overrides
```

This pattern enables:
- **Immediate functionality** — AAI works out of the box with sensible defaults
- **Personal customization** — Users can override any default without modifying core files
- **Clean updates** — AAI updates don't overwrite personal configurations
- **Privacy separation** — USER content is never synced to the public AAI repository

---

## The Lookup Pattern

When AAI needs configuration, it follows a cascading lookup:
...
```
AAI/AAISECURITYSYSTEM/         # SYSTEM tier (base)
├── README.md                          # Overview
...
AAI/USER/AAISECURITYSYSTEM/                # USER tier (personal)
├── patterns.yaml                      # Your security rules
├── QUICKREF.md                        # Your quick reference
└── ...
```

The SecurityValidator hook checks `AAI/USER/AAISECURITYSYSTEM/patterns.yaml` first, falling back to `AAI/AAISECURITYSYSTEM/patterns.example.yaml`.

### Response Format

```
AAI/RESPONSEFORMAT.md         # SYSTEM tier (base format rules)
AAI/USER/RESPONSEFORMAT.md    # USER tier (personal overrides)
```
...
Private skills use the `_ALLCAPS` prefix and are never synced to public AAI.
...
The SYSTEM tier must always provide functional defaults. A fresh AAI installation should work immediately without requiring USER configuration.
...
The `USER/` directory is excluded from public AAI sync. Anything in USER:
- Never appears in public AAI repository
...
When AAI updates, only SYSTEM tier files change. Your USER configurations remain untouched. This means:
- Safe to update AAI without losing customizations
...
const USER_PATTERNS_PATH = paiPath('AAI', 'USER', 'AAISECURITYSYSTEM', 'patterns.yaml');
const SYSTEM_PATTERNS_PATH = paiPath('AAI', 'AAISECURITYSYSTEM', 'patterns.example.yaml');
...
Future AAI versions may support declarative merging.
...
- `AAISECURITYSYSTEM/` — Security system architecture and patterns
- `SKILLSYSTEM.md` — Skill naming conventions (public vs private)
- `AAISYSTEMARCHITECTURE.md` — Overall AAI architecture

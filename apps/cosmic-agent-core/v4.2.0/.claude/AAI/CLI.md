...
# AAI Command-Line Tools

AAI provides two CLI tools for running infrastructure from the terminal:

1. **The Algorithm CLI** — Run the AAI Algorithm against PRDs in loop or interactive mode
...
**Location:** `~/.claude/AAI/Tools/algorithm.ts`

The Algorithm CLI executes the AAI Algorithm (Observe → Think → Plan → Build → Execute → Verify → Learn) against PRD files. It supports two modes: autonomous loop execution (no human needed) and interactive sessions (human-in-the-loop).

### Quick Start

```bash
# Run the Algorithm in autonomous loop mode
bun ~/.claude/AAI/Tools/algorithm.ts -m loop -p <PRD-path> -n 20

# Run in interactive mode (launches a claude session with PRD context)
bun ~/.claude/AAI/Tools/algorithm.ts -m interactive -p <PRD-path>

# Check status of all PRDs
bun ~/.claude/AAI/Tools/algorithm.ts status
```
...
bun ~/.claude/AAI/Tools/algorithm.ts -m loop -p PRD-20260213-auth.md

# Fast loop — 20 max iterations
bun ~/.claude/AAI/Tools/algorithm.ts -m loop -p PRD-20260213-auth.md -n 20

# Parallel loop — 4 agents working on different criteria simultaneously
bun ~/.claude/AAI/Tools/algorithm.ts -m loop -p PRD-20260213-auth.md -n 20 -a 4
```
...
```bash
bun ~/.claude/AAI/Tools/algorithm.ts -m interactive -p PRD-20260213-feature.md
```
...
bun ~/.claude/AAI/Tools/algorithm.ts status -p PRD-20260213-auth

# Pause a running loop (loop checks between iterations)
bun ~/.claude/AAI/Tools/algorithm.ts pause -p PRD-20260213-auth

# Resume a paused loop
bun ~/.claude/AAI/Tools/algorithm.ts resume -p PRD-20260213-auth

# Stop a loop permanently
bun ~/.claude/AAI/Tools/algorithm.ts stop -p PRD-20260213-auth
```
...
The Algorithm CLI integrates with the AAI dashboard by writing state to `MEMORY/STATE/algorithms/`:
...
**Location:** `~/.claude/AAI/ACTIONS/pai.ts`
...
**Location:** `~/.claude/AAI/ACTIONS/lib/runner.v2.ts`
...
**Location:** `~/.claude/AAI/ACTIONS/lib/pipeline-runner.ts`
...
```bash
# The Algorithm CLI
alias algorithm="bun ~/.claude/AAI/Tools/algorithm.ts"

# The Arbol CLI
alias aai="bun ~/.claude/AAI/ACTIONS/pai.ts"

# Runners (optional — aai CLI wraps these)
alias arbol-run="bun ~/.claude/AAI/ACTIONS/lib/runner.v2.ts"
alias arbol-pipe="bun ~/.claude/AAI/ACTIONS/lib/pipeline-runner.ts"
```
...
|------|---------|---------|
| **Algorithm CLI** | Run AAI Algorithm against PRDs | `bun Tools/algorithm.ts -m loop -p <PRD>` |

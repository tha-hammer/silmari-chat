# AAI Tools - CLI Utilities Reference
...
**Location:** `~/.claude/AAI/Tools/Inference.ts`
...
bun ~/.claude/AAI/Tools/Inference.ts --level fast "System prompt" "User prompt"

# Standard (Sonnet) - balanced reasoning, typical analysis
bun ~/.claude/AAI/Tools/Inference.ts --level standard "System prompt" "User prompt"

# Smart (Opus) - deep reasoning, strategic decisions
bun ~/.claude/AAI/Tools/Inference.ts --level smart "System prompt" "User prompt"

# With JSON output
bun ~/.claude/AAI/Tools/Inference.ts --json --level fast "Return JSON" "Input"

# Custom timeout
bun ~/.claude/AAI/Tools/Inference.ts --level standard --timeout 60000 "Prompt" "Input"
```
...
**Location:** `~/.claude/AAI/Tools/RemoveBg.ts`
...
bun ~/.claude/AAI/Tools/RemoveBg.ts /path/to/image.png

# Remove background and save to different path
bun ~/.claude/AAI/Tools/RemoveBg.ts /path/to/input.png /path/to/output.png

# Process multiple images
bun ~/.claude/AAI/Tools/RemoveBg.ts image1.png image2.png image3.png
```

**Environment Variables:**
- `REMOVEBG_API_KEY` - Required for background removal (from `${AAI_DIR}/.env`)
...
**Location:** `~/.claude/AAI/Tools/AddBg.ts`
...
bun ~/.claude/AAI/Tools/AddBg.ts /path/to/transparent.png "#EAE9DF" /path/to/output.png

# Add brand background color
bun ~/.claude/AAI/Tools/AddBg.ts /path/to/transparent.png --brand /path/to/output.png
```
...
**Location:** `~/.claude/AAI/Tools/GetTranscript.ts`
...
bun ~/.claude/AAI/Tools/GetTranscript.ts "https://www.youtube.com/watch?v=VIDEO_ID"

# Save transcript to file
bun ~/.claude/AAI/Tools/GetTranscript.ts "https://www.youtube.com/watch?v=VIDEO_ID" --save /path/to/transcript.txt
```
...
**Location:** `~/.claude/AAI/Tools/extract-transcript.py`
...
**Location:** `~/.claude/AAI/Tools/YouTubeApi.ts`
...
bun ~/.claude/AAI/Tools/YouTubeApi.ts --channel-stats

# Get video statistics
bun ~/.claude/AAI/Tools/YouTubeApi.ts --video-stats VIDEO_ID

# Get latest uploads
bun ~/.claude/AAI/Tools/YouTubeApi.ts --latest-videos
```

**Environment Variables:**
- `YOUTUBE_API_KEY` - Required for API access (from `${AAI_DIR}/.env`)
...
1. **Add tool file:** Place `.ts` or `.py` file directly in `~/.claude/AAI/Tools/`
...
2. **Document here:** Add section to this file with:
   - Tool location (e.g., `~/.claude/AAI/Tools/ToolName.ts`)
   - Usage examples
   - When to use triggers
   - Environment variables (if any)

3. **Update AAI/SKILL.md:** Ensure SYSTEM/TOOLS.md is in the documentation index

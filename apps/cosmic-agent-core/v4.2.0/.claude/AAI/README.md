# AAI — Agent Assistant Infrastructure

AAI is a general problem-solving system that magnifies human capabilities. It runs inside Claude Code as an interconnected set of skills, hooks, tools, memory, and configuration — all orchestrated by The Algorithm.
...
**This directory (`AAI/`)** contains all system documentation, tools, user context, and the SKILL.md that defines AAI as a skill. The rest of the system lives alongside it under `~/.claude/` (hooks, skills, settings, memory).
...
~/.claude/
  ...
  AAI/                         # This directory — system docs + tools + user context
    Algorithm/                 # Versioned algorithm files + LATEST pointer
```

## Core Subsystems

### The Algorithm (`AAI/Algorithm/`)
...
### Tools (`Tools/`)
TypeScript utilities in `AAI/Tools/`: `BuildCLAUDE.ts` (generate CLAUDE.md from template), `Inference.ts` (AI calls), `GenerateSkillIndex.ts`, `SessionProgress.ts`, `Banner.ts`, and more.

### Agents (`AAIAGENTSYSTEM.md`)
...
- **Projects:** `PROJECTS/`
- **Life Goals:** `LifeOS/` (via LifeOS skill)
- **Work:** `WORK/`, `BUSINESS/`
...
|--------|--------|---------|---------|
| `CLAUDE.md` | `CLAUDE.md.template` + `settings.json` + `AAI/Algorithm/LATEST` | `bun AAI/Tools/BuildCLAUDE.ts` | SessionStart hook + manual |
...
- **Add user context:** Create files in `AAI/USER/`

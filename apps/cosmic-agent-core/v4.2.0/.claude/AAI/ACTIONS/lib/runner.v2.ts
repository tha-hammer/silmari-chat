...
/**
 * ============================================================================
 * AAI ACTIONS v2 - Runner with Capability Injection
 * ============================================================================
 *
...
/**
 * Shell capability conforming to the ActionContext ShellResult contract
 * ({stdout, stderr, exitCode, signal}). Exported for unit testing.
 *
 * Two bugs this fixes (every shell-based action read `r.exitCode`, but the old
 * impl returned `code`, so `r.exitCode` was always `undefined` → actions threw
 * "subprocess exited undefined" on EVERY call, even on success):
 *   1. Return `exitCode` (+ `signal`), not just `code`. `code` is kept as an
 *      alias for the legacy types.v2 ShellResult shape.
 *   2. Use `.nothrow()` so stdout is preserved on non-zero exit. The old
 *      try/catch dropped stdout to "" on failure, so actions that parse a
 *      summary from stdout on exit 2/3 (e.g. A_REEL_COMPOSE fail/clarify) got
 *      empty stdout → SUMMARY_PARSE_FAILED.
 */
export function createShellCapability(): NonNullable<ActionCapabilities["shell"]> {

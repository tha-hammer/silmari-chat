...
/**
 * BannerNeofetch - Modern Neofetch-Style AAI Banner
 *
 ...
 * BOTTOM SECTION: Gradient header, quote, sparkline histogram, AAI block art
 *
...
function colorLogo(lines: string[]): string[] {
  return lines.map((line, i) => {
    ...
    // Special handling for AAI text
    if (line.includes("AAI") || line.includes("P A I")) {
...
// Color AAI block art with gradient (P=blue, A=purple, I=cyan)
function colorPaiArt(): string[] {
  return AAI_BLOCK_ART.map(line => {
...
function createNeofetchBanner(): string {
  ...
  statsLines.push(`${gradientName} ${UI.muted}@${RESET} ${UI.subtext}Agent Assistant Infrastructure${RESET}`);
  ...
  // Gradient header: AAI | Agent Assistant Infrastructure
  const paiGradient = `${BOLD}${GRADIENT.blue1}P${RESET}${BOLD}${GRADIENT.purple1}A${RESET}${BOLD}${GRADIENT.cyan1}I${RESET}`;
  const headerContent = `${paiGradient} ${UI.dim}│${RESET} ${UI.text}Agent Assistant Infrastructure${RESET}`;
  lines.push(`  ${UI.dim}│${RESET}${center(headerContent, bottomWidth - 2)}${UI.dim}│${RESET}`);

  // Quote
  const quote = `${ITALIC}${UI.subtext}"Maximizing human potential through structured intelligence..."${RESET}`;
  ...
  const githubUrl = `${linkIcon} ${UI.subtext}github.com/${RESET}${GRADIENT.blue1}danielmiessler${RESET}${UI.subtext}/${RESET}${GRADIENT.purple1}AAI${RESET}`;
...
function createCompactBanner(): string {
  ...
  lines.push("");
  lines.push(`  ${UI.dim}╭──${RESET} ${paiGradient} ${UI.dim}│${RESET} ${UI.subtext}Agent Assistant Infrastructure${RESET} ${UI.dim}──────╮${RESET}`);

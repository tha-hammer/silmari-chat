#!/usr/bin/env bun
/**
 * ActivityParser - Parse session activity for AAI repo update documentation
 *
 * Commands:
 *   --today              Parse all today's activity
 *   --session <id>       Parse specific session only
 *   --generate           Generate MEMORY/AAISYSTEMUPDATES/ file (outputs path)
 *
 * Examples:
...
const SYSTEM_UPDATES_DIR = path.join(MEMORY_DIR, "AAISYSTEMUPDATES");  // Canonical system change history
...
const PATTERNS = {
  ...
  skip: [
    /MEMORY\/AAISYSTEMUPDATES\//,    // Don't self-reference
  ...
  hooks: /hooks\/.*\.ts$/,
  architecture: /(ARCHITECTURE|AAISYSTEMARCHITECTURE|SKILLSYSTEM)\.md$/i,
...
async function writeUpdateFile(activity: ParsedActivity): Promise<string> {
  ...
  // Structure: MEMORY/AAISYSTEMUPDATES/YYYY/MM/YYYY-MM-DD_title.md
...
if (values.help) {
  console.log(`
ActivityParser - Parse session activity for AAI repo updates

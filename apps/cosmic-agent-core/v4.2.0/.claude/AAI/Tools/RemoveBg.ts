 ...
 * Part of the Images skill for AAI system.
 *
 * Usage:
...
async function loadEnv(): Promise<void> {
  const envPath = process.env.AAI_CONFIG_DIR ? resolve(process.env.AAI_CONFIG_DIR, ".env") : resolve(process.env.HOME!, ".config/AAI/.env");
  try {
...
async function removeBackground(
  ...
  if (!apiKey) {
    ...
    console.error("   Add it to ${AAI_DIR}/.env or export it in your shell");
    process.exit(1);
  }

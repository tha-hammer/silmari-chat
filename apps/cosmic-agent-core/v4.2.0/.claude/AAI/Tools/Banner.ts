...
/**
 * AAI Banner - Dynamic Multi-Design Neofetch Banner
...
function getStats(): SystemStats {
  ...
  let repoUrl = "github.com/hackerman-cosmic/Agent-Assistant-Infrastructure";
  try {
    const settings = JSON.parse(readFileSync(join(CLAUDE_DIR, "settings.json"), "utf-8"));
    name = settings.daidentity?.displayName || settings.daidentity?.name || "AAI";
...
function createNavyBanner(stats: SystemStats, width: number): string {
  ...
  // AAI logo - 2x scale (20 wide × 10 tall), same proportions
  ...
  const infoLines = [
    ...
    `${C.navy}\u2B22${RESET}  ${C.slate}AAI${RESET}       ${C.silver}${stats.paiVersion}${RESET}`,                            // ⬢ hexagon (tech/AI)
  ...
  // Header: AAI (in logo colors) | Agent Assistant Infrastructure
  const paiColored = `${C.navy}P${RESET}${C.medBlue}A${RESET}${C.lightBlue}I${RESET}`;
  const headerText = `${paiColored} ${C.steel}|${RESET} ${C.slate}Agent Assistant Infrastructure${RESET}`;
  const headerLen = 33; // "AAI | Agent Assistant Infrastructure"
  ...
  const quote = `${ITALIC}${C.lightBlue}"Maximizing human potential..."${RESET}`;
...
function createElectricBanner(stats: SystemStats, width: number): string {
  ...
  // AAI logo - matching reference image exactly
  const B = "\u2588";
  const logo = [
  ...
  const infoLines = [
    ...
    `${P.neonBlue}${SYM.target}${RESET} ${P.glow}OS${RESET}         ${P.electric}AAI ${stats.paiVersion}${RESET}`,
...
function createTealBanner(stats: SystemStats, width: number): string {
  ...
  const infoLines = [
    ...
    `${P.mediumTeal}${SYM.wave}${RESET} ${P.paleAqua}OS${RESET}         ${P.aquamarine}AAI ${stats.paiVersion}${RESET}`,
...
function createIceBanner(stats: SystemStats, width: number): string {
  ...
  const infoLines = [
    ...
    `${P.iceBlue}${SYM.crystal}${RESET} ${P.frost}OS${RESET}         ${P.white}AAI ${stats.paiVersion}${RESET}`,
...
function createNavyMediumBanner(stats: SystemStats, width: number): string {
  ...
  const infoLines = [
    `${C.slate}"${RESET}${C.lightBlue}${stats.catchphrase}${RESET}${C.slate}..."${RESET}`,
    `${C.steel}${BOX.h.repeat(24)}${RESET}`,
    `${C.navy}\u2B22${RESET}  ${C.slate}AAI${RESET}       ${C.silver}${stats.paiVersion}${RESET}`,
...
function createNavyCompactBanner(stats: SystemStats, width: number): string {
  ...
  const infoLines = [
    ...
    `${C.navy}\u2B22${RESET} ${C.slate}AAI${RESET} ${C.silver}${stats.paiVersion}${RESET} ${C.navy}\u2699${RESET} ${C.silver}${stats.algorithmVersion}${RESET}`,

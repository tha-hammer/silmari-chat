...
/**
 * BannerMatrix - Matrix Digital Rain AAI Banner
 * Neofetch-style layout with The Matrix aesthetic
 *
 * Design:
 *   LEFT:  AAI logo emerging from Matrix rain (Katakana cascade)
 *   RIGHT: System stats as terminal readout
 *   BOTTOM: Glitched branding + AAI in dripping Matrix style
 *
...
// =============================================================================
// AAI Logo in Matrix Style (ASCII that emerges from rain)
// =============================================================================

// Large AAI letters that will "emerge" from the rain
const AAI_MATRIX_LOGO = [
  " ██████╗   █████╗  ██╗",
  " ██╔══██╗ ██╔══██╗ ██║",
  " ██████╔╝ ███████║ ██║",
  " ██╔═══╝  ██╔══██║ ██║",
...
// Compact AAI logo for smaller modes
const AAI_COMPACT = [
  "┌───┐┌───┐┌─┐",
  "│ ┌─┘│ ┌─┤│ │",
  "│ │  │ ├─┤│ │",
...
function createNanoBanner(stats: SystemStats): string {
  ...
  return `${d}ｱ${RESET}${g}AAI${RESET}${d}ｲ${RESET} ${w}${stats.name}${RESET} ${g}[ON]${RESET}`;
}
 ...
 * LEFT: AAI logo with rain | RIGHT: System stats
 * BOTTOM: AAI dripping + branding
 */
function createNormalBanner(stats: SystemStats): string {
  ...
  // Dimensions
  const logoWidth = 40;  // Width for AAI logo + rain
  ...
  // AAI logo lines with rain surrounding
  const paiLines = AAI_MATRIX_LOGO.map((line, i) => {
  ...
  const brandLine1 = `${d}  "Maximizing human potential through intelligent automation..."${RESET}`;
  lines.push(rainOverlay(brandLine1.padEnd(width), 0.15));

  // AAI dripping effect
  const paiDripLines = AAI_DRIP.slice(0, 4);  // Just first 4 lines for compactness
  ...
  const urlLine = `${d}$ git clone ${RESET}${g}https://github.com/hackerman-cosmic/Agent-Assistant-Infrastructure${RESET}`;
  const urlCentered = " ".repeat(Math.floor((width - 50) / 2));
  lines.push(rainOverlay(`${urlCentered}${urlLine}`.padEnd(width), 0.1));

  // Frame bottom
  const bottomFrame = `${f}${BOLD}${"=".repeat(dividerCol - 1)}[${RESET}${p}AAI${RESET}${f}${BOLD}]${"=".repeat(width - dividerCol - 5)}${RESET}`;

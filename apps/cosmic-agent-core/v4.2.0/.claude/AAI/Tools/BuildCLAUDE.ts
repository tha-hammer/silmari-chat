 ...
 * and AAI/Algorithm/LATEST, writes CLAUDE.md.
 *
 * Called by:
 *   - AAI installer (first install)
 *   - SessionStart hook (keeps fresh automatically)
 *   - Manual: bun AAI/Tools/BuildCLAUDE.ts
 */

import { readFileSync, writeFileSync, existsSync } from "fs";
import { join } from "path";

const AAI_DIR = join(process.env.HOME!, ".claude");
const TEMPLATE_PATH = join(AAI_DIR, "CLAUDE.md.template");
const OUTPUT_PATH = join(AAI_DIR, "CLAUDE.md");
const SETTINGS_PATH = join(AAI_DIR, "settings.json");
const ALGORITHM_DIR = join(AAI_DIR, "AAI/Algorithm");
...
function getAlgorithmVersion(): string {
  if (!existsSync(LATEST_PATH)) {
    console.error("⚠ AAI/Algorithm/LATEST not found, defaulting to v3.7.0");
    return "v3.7.0";
  }
...
function loadVariables(): Record<string, string> {
  ...
  return {
    ...
    "{{AAI_VERSION}}": settings.pai?.version || "4.0.3",
    "{{ALGO_VERSION}}": algoVersion,
    "{{ALGO_PATH}}": `AAI/Algorithm/${algoVersion}.md`,
  };
}
...
export function needsRebuild(): boolean {
  ...
  const algoVersion = getAlgorithmVersion();
  const algoPathPattern = /AAI\/Algorithm\/(.+?)\.md/;

 ...
 *
 * Output:
 * - Creates AAISYSTEMUPDATES entry with AI-generated narrative
...
const AAI_DIR = process.env.HOME + '/.claude';
const CREATE_UPDATE_SCRIPT = join(AAI_DIR, 'skills/_SYSTEM/Tools/CreateUpdate.ts');
...
function generateDescriptiveTitle(changes: FileChange[]): string {
  ...
  const hasAAISystem = paths.some(p => p.includes('/AAI/'));
  const hasAAIUser = paths.some(p => p.includes('AAI/USER/'));
  ...
  // AAI system changes
  else if (hasAAISystem) {
    const docNames = paths
      .filter(p => p.includes('/AAI/'))
      .map(p => basename(p, '.md'));
...
async function generateNarrativeWithAI(
  ...
  const prompt = `You are analyzing a Claude Code session to generate documentation for a AAI (Agent Assistant Infrastructure) system update.
...
async function generateVerboseNarrative(
  ...
  const skillContext = skillNames.size > 0 ? [...skillNames].slice(0, 3).join(', ') : 'AAI system';

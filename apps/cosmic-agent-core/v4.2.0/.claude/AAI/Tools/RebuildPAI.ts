...
/**
 * RebuildAAI.ts - Assembles SKILL.md from Components/
 *
 * Usage: bun ~/.claude/AAI/Tools/RebuildAAI.ts
 *
...
const AAI_DIR = join(HOME, ".claude/AAI");
const COMPONENTS_DIR = join(AAI_DIR, "Components");
const ALGORITHM_DIR = join(COMPONENTS_DIR, "Algorithm");
const OUTPUT_FILE = join(AAI_DIR, "SKILL.md");
...
function loadVariables(): Record<string, string> {
  try {
    ...
    return {
      "{DAIDENTITY.NAME}": settings.daidentity?.name || "AAI",
      "{DAIDENTITY.FULLNAME}": settings.daidentity?.fullName || "Personal AI",
      "{DAIDENTITY.DISPLAYNAME}": settings.daidentity?.displayName || "AAI",
...
for (const file of components) {
  ...
  if (file === "00-frontmatter.md") {
    content = content.replace(
      "  Build:  bun ~/.claude/AAI/Tools/RebuildAAI.ts",
      `  Build:  bun ~/.claude/AAI/Tools/RebuildAAI.ts\n  Built:  ${timestamp}`
    );
  }

...
const BANNER_SCRIPT = join(CLAUDE_DIR, "AAI", "Tools", "Banner.ts");
...
async function cmdLaunch(options: { mcp?: string; resume?: boolean; skipPerms?: boolean; local?: boolean }) {
  ...
  // (InstantiateAAI.ts is retired — kept for reference only)
  ...
  // Change to AAI directory unless --local flag is set
  if (!options.local) {

...
function getCounts(): Counts {
  // === v4.1.0: Beads-first counts ===
  try {
    const { isBeadsAvailable, brList } = require('../../hooks/lib/beads-index');
    if (isBeadsAvailable()) {
      const work = brList({ labels: ['note:artifact', 'kind:prd'], limit: 0 });
      const learnings = brList({ labels: ['kind:learning'], limit: 0 });
      const signals = brList({ labels: ['memory:signal'], limit: 0 });
      const failures = brList({ labels: ['kind:failure-pattern'], limit: 0 });
      // Use beads counts where available, fall back to filesystem for the rest
      return {
        skills: countSkills(),
        workflows: countWorkflowFiles(join(PAI_DIR, "skills")),
        hooks: countHooks(),
        signals: signals.length > 0 ? signals.length : countFilesRecursive(join(PAI_DIR, "MEMORY/LEARNING"), ".md"),
        files: countFilesRecursive(join(PAI_DIR, "PAI/USER")),
        work: work.length > 0 ? work.length : (() => {
          let count = 0;
          try {
  ...
  } catch { /* fall through to filesystem counts */ }

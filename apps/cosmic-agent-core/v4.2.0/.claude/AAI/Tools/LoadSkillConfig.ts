 ...
 *
 * Usage:
 *   import { loadSkillConfig } from '~/.claude/AAI/Tools/LoadSkillConfig';
 *   const config = loadSkillConfig<MyConfigType>(__dirname, 'config.json');
 *
 * Or CLI:
 *   bun ~/.claude/AAI/Tools/LoadSkillConfig.ts <skill-dir> <filename>
 */
...
// Constants
const HOME = homedir();
const CUSTOMIZATION_DIR = join(HOME, '.claude', 'AAI', 'USER', 'SKILLCUSTOMIZATIONS');
...
Examples:
  bun LoadSkillConfig.ts ~/.claude/skills/AAIUpgrade sources.json
  bun LoadSkillConfig.ts --list
  bun LoadSkillConfig.ts --check AAIUpgrade
`);
    process.exit(0);
  }

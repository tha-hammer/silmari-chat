...
const AAI_DIR = process.env.AAI_DIR || join(process.env.HOME!, '.claude');
...
function loadRecentNotes(daysBack: number = 7): RelationshipNote[] {
  ...
  for (const monthStr of months) {
    const monthDir = join(AAI_DIR, 'MEMORY/RELATIONSHIP', monthStr);
...
function loadRecentRatings(daysBack: number = 7): Array<{ rating: number; date: string }> {
  const ratingsPath = join(AAI_DIR, 'MEMORY/LEARNING/SIGNALS/ratings.jsonl');
...
function parseOpinions(): Map<string, { confidence: number; section: string }> {
  ...
  const opinionsPath = join(AAI_DIR, 'AAI/USER/OPINIONS.md');
...
function checkMilestones(notes: RelationshipNote[]): string[] {
  ...
  const storyPath = join(AAI_DIR, 'AAI/USER/OUR_STORY.md');

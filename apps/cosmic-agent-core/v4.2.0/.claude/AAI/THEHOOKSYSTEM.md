...
> **AAI 4.0** — This system is under active development. APIs, configuration formats, and features may change without notice.
...
The AAI hook system is an event-driven automation infrastructure built on Claude Code's native hook support. Hooks are executable scripts (TypeScript/Python) that run automatically in response to specific events during Claude Code sessions.
...
**Use Cases:**
- Load AAI context from `AAI/SKILL.md`
  ...
  "SessionStart": [
    {
      "hooks": [
        {
          "type": "command",
          "command": "${AAI_DIR}/hooks/KittyEnvPersist.hook.ts"
        },
        {
          "type": "command",
          "command": "${AAI_DIR}/hooks/LoadContext.hook.ts"
        }
      ]
    }
  ]
  ...
  "SessionEnd": [
    {
      "hooks": [
        {
          "type": "command",
          "command": "${AAI_DIR}/hooks/WorkCompletionLearning.hook.ts"
        },
        {
          "type": "command",
          "command": "${AAI_DIR}/hooks/SessionCleanup.hook.ts"
        },
        {
          "type": "command",
          "command": "${AAI_DIR}/hooks/RelationshipMemory.hook.ts"
        },
        {
          "type": "command",
          "command": "${AAI_DIR}/hooks/UpdateCounts.hook.ts"
        },
        {
          "type": "command",
          "command": "${AAI_DIR}/hooks/IntegrityCheck.hook.ts"
        }
      ]
    }
  ]
  ...
  "UserPromptSubmit": [
    {
      "hooks": [
        {
          "type": "command",
          "command": "${AAI_DIR}/hooks/RatingCapture.hook.ts"
        },
        {
          "type": "command",
          "command": "${AAI_DIR}/hooks/UpdateTabTitle.hook.ts"
        },
        {
          "type": "command",
          "command": "${AAI_DIR}/hooks/SessionAutoName.hook.ts"
        }
      ]
    }
  ]
...
- **Inference:** `import { inference } from '../AAI/Tools/Inference'` → `inference({ level: 'fast', expectJson: true })`
...
- **Inference:** `import { inference } from '../AAI/Tools/Inference'` → `inference({ level: 'fast' })`
...
```json
{
  "Stop": [
    {
      "hooks": [
        { "type": "command", "command": "${AAI_DIR}/hooks/LastResponseCache.hook.ts" },
        { "type": "command", "command": "${AAI_DIR}/hooks/ResponseTabReset.hook.ts" },
        { "type": "command", "command": "${AAI_DIR}/hooks/VoiceCompletion.hook.ts" },
        { "type": "command", "command": "${AAI_DIR}/hooks/DocIntegrity.hook.ts" },
        { "type": "command", "command": "${AAI_DIR}/hooks/AlgorithmTab.hook.ts" }
      ]
    }
  ]
}
...
```json
{
  "PreToolUse": [
    ...
    },
    {
      ...
      "hooks": [
        { "type": "command", "command": "${AAI_DIR}/hooks/SetQuestionTab.hook.ts" }
      ]
    },
    {
      "matcher": "Task",
      "hooks": [
        { "type": "command", "command": "${AAI_DIR}/hooks/AgentExecutionGuard.hook.ts" }
      ]
    },
    {
      "matcher": "Skill",
      "hooks": [
        { "type": "command", "command": "${AAI_DIR}/hooks/SkillGuard.hook.ts" }
      ]
    }
  ]
}
  ...
  "PostToolUse": [
    {
      "matcher": "AskUserQuestion",
      "hooks": [
        { "type": "command", "command": "${AAI_DIR}/hooks/QuestionAnswered.hook.ts" }
      ]
    },
    {
      "matcher": "Write",
      "hooks": [
        { "type": "command", "command": "${AAI_DIR}/hooks/PRDSync.hook.ts" }
      ]
    },
    {
      "matcher": "Edit",
...
**Key Variables:**
- `AAI_DIR` - AAI installation directory (typically `~/.claude`)
- Hook scripts reference `${AAI_DIR}` in command paths
...
- Central to the AAI install wizard
    ...
    "HookEventName": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "${AAI_DIR}/hooks/my-hook.ts --arg value"
          }
        ]
      }
    ]
...
**Full Documentation:** See `~/.claude/AAI/TERMINALTABS.md`
    ...
    "Stop": [
      {
        "hooks": [
          {
            "type": "command",
            "command": "${AAI_DIR}/hooks/my-custom-hook.ts"
          }
        ]
      }
    ]
...
- Use `${AAI_DIR}` in settings.json for portability
- Access in code via `process.env.AAI_DIR`
...
2. Is path correct in settings.json? Use `${AAI_DIR}/hooks/...`
...
2. Is voice_id correct? See `AAI/SKILL.md` for mappings
3. Is message format correct? `{"message":"...", "voice_id":"...", "title":"..."}`
4. Is ElevenLabs API key in `${AAI_DIR}/.env`?
...
**Check:**
1. Does `~/.claude/AAI/SKILL.md` exist?
2. Is `LoadContext.hook.ts` executable?
3. Is `AAI_DIR` env variable set correctly?
...
- File not found → Check `AAI_DIR` environment variable
...
```json
{
  "Stop": [
    {
      "hooks": [
        { "command": "${AAI_DIR}/hooks/Stop hooks.hook.ts" }  // Single orchestrator
      ]
    }
  ]
}
...
- **History/Memory:** `~/.claude/AAI/MEMORYSYSTEM.md`
...
Path: ~/.claude/AAI/Tools/Inference.ts
Import: import { inference } from '../AAI/Tools/Inference'
...
### `AAI/Tools/Inference.ts`
Unified AI inference with three run levels.

```typescript
import { inference } from '../AAI/Tools/Inference';
...
| File | Purpose |
|------|---------|
| `${AAI_DIR}/hooks/lib/event-types.ts` | TypeScript discriminated union of all AAI event types (22 interfaces covering algorithm, work, session, rating, learning, voice, PRD, doc, build, system, tab, hook error, and custom events) |
| `${AAI_DIR}/hooks/lib/event-emitter.ts` | `appendEvent()` utility that writes typed events to `${AAI_DIR}/MEMORY/STATE/events.jsonl` |

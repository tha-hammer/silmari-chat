...
<!--
AAI SYSTEM ARCHITECTURE TEMPLATE
================================
This file defines the GENERIC architecture patterns for any Agent Assistant Infrastructure.
These are the foundational patterns that apply to ALL AAI implementations.
...
**The Founding Principles and Universal Architecture Patterns for Agent Assistant Infrastructure**

This document defines the foundational architecture that applies to ALL AAI implementations. For user-specific customizations, see `USER/ARCHITECTURE.md`.

---

## Core Philosophy

**AAI is scaffolding for AI, not a replacement for human intelligence.**

The system is designed on the principle that **AI systems need structure to be reliable**. Like physical scaffolding supports construction work, AAI provides the architectural framework that makes AI assistance dependable, maintainable, and effective.
...
**AAI exists to help you accomplish your goals in life—and perform the work required to get there.**

The most powerful AI systems are being built inside companies for companies. AAI democratizes access to **personalized agentic infrastructure**—a system that knows your goals, preferences, context, and history, and uses that understanding to help you more effectively.

**What makes AAI personal:**
- **Your Goals** — LifeOS captures your mission, strategies, beliefs, and what you're working toward
...
**Key Takeaway:** AI should magnify everyone. AAI is the infrastructure that makes AI truly personal.

### 2. The Continuously Upgrading Algorithm (THE CENTERPIECE)

**This is the gravitational center of AAI—everything else exists to serve it.**

AAI is built around a universal algorithm for accomplishing any task: **Current State → Ideal State** via verifiable iteration. This pattern applies at every scale—fixing a typo, building a feature, launching a company, human flourishing.
...
All of this feeds back into improving **The Algorithm itself**. AAI is not a static tool—it is a continuously upgrading algorithm that gets better at helping you with every interaction.
...
A well-structured system with good scaffolding will outperform a more powerful model with poor structure. AAI's value comes from:
...
**Key Takeaway:** Skills are how AAI scales - each new domain gets its own skill.
...
```
┌─────────────────┐
│  Session Start  │──► Load AAI context
└─────────────────┘
...
└── AAISYSTEMUPDATES/     # System change documentation
```
...
AAI actions and pipelines run in two environments with identical behavior:
...
**Arbol** is the Cloudflare Workers deployment of AAI's action/pipeline system. It follows the UNIX philosophy — each action is a separate Worker that does one thing, pipelines are Workers that chain actions via service bindings.
...
**Full documentation:** `AAI/ACTIONS/README.md`, `AAI/PIPELINES/README.md`
...
```
PRIVATE: ~/.claude/                    PUBLIC: ${PROJECTS_DIR}/AAI/
...
**AAI manages its own integrity, security, and documentation through the System skill.**

The System skill is the centralized mechanism for AAI self-management. It ensures the infrastructure remains healthy, secure, and well-documented.
...
| **Documentation Updates** | Records system changes to MEMORY/AAISYSTEMUPDATES/ | `DocumentChanges.md` |
...
|-----------|----------|------------------|
| `AAI/USER/` | Personal data, finances, health, contacts | RESTRICTED |
| `AAI/WORK/` | Customer data, consulting, client deliverables | RESTRICTED |

**Rule:** Content from USER/ and WORK/ must NEVER appear outside of them or in the public AAI repository.

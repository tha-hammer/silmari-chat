...
Universal behavioral rules for AAI. Force-loaded at session start via `settings.json → loadAtStartup`.
...
**AAI Inference Tool for AI calls.** Use `bun Tools/Inference.ts fast|standard|smart`, never import `@anthropic-ai/sdk` directly.

...
├── AAISYSTEMUPDATES/         # Architecture change history
│   ├── index.json
│   ├── CHANGELOG.md
│   └── YYYY/MM/
└── README.md
...
This is the actual "firehose" - every message, tool call, and response. AAI leverages this native storage rather than duplicating it.
...
**Structure:**
- `LEARNING/SYSTEM/YYYY-MM/` - AAI/tooling learnings (infrastructure issues)
...
bun run ~/.claude/AAI/Tools/FailureCapture.ts --migrate
```
...
bun run ~/.claude/AAI/Tools/SessionHarvester.ts --recent 10

# Generate pattern synthesis
bun run ~/.claude/AAI/Tools/LearningPatternSynthesis.ts --week
```
...
- **Architecture:** `AAISYSTEMARCHITECTURE.md`

#!/usr/bin/env bun
/**
 * LoadContext.hook.ts - Inject AAI dynamic context into Claude's Context (SessionStart)
 *
 * AAI v4.0: Core context (identity, rules, format) is now in CLAUDE.md and loaded
 ...
 * INPUT:
 * - Environment: AAI_DIR
 * - Files: AAI/USER/OPINIONS.md, MEMORY/RELATIONSHIP/*, MEMORY/LEARNING/*,
...
function loadRelationshipContext(paiDir: string): string | null {
  ...
  const opinionsPath = join(paiDir, 'AAI/USER/OPINIONS.md');
...
*Full details: AAI/USER/OPINIONS.md, MEMORY/RELATIONSHIP/*
`;
}
...
async function checkActiveProgress(paiDir: string): Promise<string | null> {
  ...
  summary += '\n💡 To resume project: `bun run ~/.claude/AAI/Tools/SessionProgress.ts resume <project>`\n';
  summary += '💡 To complete project: `bun run ~/.claude/AAI/Tools/SessionProgress.ts complete <project>`\n';
...
async function main() {
  try {
    ...
    // === v4.2.0: Multi-mode context assembly (replaces v4.1.0 simple beads context) ===
    // REVIEW FIX (Warning #2): Reads from filesystem state, not env vars
    // REVIEW FIX (Warning #3): Falls back to v4.1.0 on any failure
    let beadsContext: string | null = null;
    try {
      const { wireContextRetrieval, readWorkContext } = await import(
        '../../../../cosmic-agent-memory/integration/hooks/wire-retrieval'
      );
      const workCtx = readWorkContext();
      const retrieval = wireContextRetrieval(workCtx);
      if (retrieval) {
        beadsContext = retrieval.rendered;
        console.error(
          `🔗 Loaded context bundle: ${retrieval.fragments.length} fragments, ` +
          `${retrieval.mode} mode (${retrieval.rendered.length} chars)`,
        );
      }
    } catch {
      // Fallback to v4.1.0 simple assembly
      try {
        const { assembleBeadsContext } = await import('./lib/beads-context');
        beadsContext = assembleBeadsContext();
        if (beadsContext) {
          console.error(`🔗 Loaded beads context [v4.1 fallback] (${beadsContext.length} chars)`);
        }
      } catch {
        console.error('⏭️ Beads context unavailable');
      }
    }

    // Inject dynamic context if we have any
    if (relationshipContext || learningContext || beadsContext) {
      const message = `<system-reminder>
AAI Dynamic Context (Auto-loaded at Session Start)
...
Dynamic context loaded. Core identity, rules, and format are in CLAUDE.md.
    ...
    // === v4.1.0: Emit context.loaded event ===
    try {
      const { getEventBus } = await import('./lib/event-bus');
      getEventBus().emit({
        type: 'context.loaded',
        sessionId: process.env.CLAUDE_SESSION_ID || '',
        fragmentCount: beadsContext ? 1 : 0,
        beadsIds: [],
        source: beadsContext ? 'beads' as const : 'filesystem' as const,
      });
    } catch { /* Silent */ }

    // === v4.2.0: Log workflow prediction at session start ===
    try {
      const { measureContextUsage } = await import('./lib/context-budget');
      const { predictWorkflowBudget, detectWorkflowType } = await import('./lib/workflow-budget');
      const snapshot = measureContextUsage();
      if (snapshot) {
        const wfType = detectWorkflowType();
        const prediction = predictWorkflowBudget(wfType, snapshot.model);
        if (prediction) {
          console.error(`📊 Budget prediction: ${wfType} sessions typically use ${prediction.estimatedContextPct.toFixed(0)}% context (${prediction.historicalCount} historical, confidence: ${prediction.confidence.toFixed(2)})`);
          if (prediction.recommendation === 'plan-handoff') {
            console.error(`⚠️ Prediction suggests planning for handoff — P75 context at ${prediction.p75ContextPct.toFixed(0)}%`);
          } else if (prediction.recommendation === 'plan-subagents') {
            console.error(`💡 Prediction suggests using subagents for heavy work — P75 context at ${prediction.p75ContextPct.toFixed(0)}%`);
          }
        } else {
          console.error(`📊 Budget: ${wfType} session on ${snapshot.model} (${(snapshot.contextWindowSize / 1000).toFixed(0)}K window) — no prediction yet`);
        }
      }
    } catch {
      // Silent — prediction is informational only
    }

    console.error('✅ AAI session initialization complete (v4.2)');
    process.exit(0);
  } catch (error) {

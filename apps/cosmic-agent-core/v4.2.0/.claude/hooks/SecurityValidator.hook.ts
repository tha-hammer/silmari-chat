...
// 1. AAI/USER/AAISECURITYSYSTEM/patterns.yaml (user's custom rules)
// 2. AAI/AAISECURITYSYSTEM/patterns.example.yaml (default template)
...
function handleBash(input: HookInput): void {
  ...
  switch (result.action) {
    case 'block':
      ...
      console.error(`[AAI SECURITY] 🚨 BLOCKED: ${result.reason}`);
    ...
    case 'confirm':
      ...
      console.log(JSON.stringify({
        decision: 'ask',
        message: `[AAI SECURITY] ⚠️ ${result.reason}\n\nCommand: ${command.slice(0, 200)}\n\nProceed?`
      }));
      break;
    ...
    case 'alert':
      ...
      console.error(`[AAI SECURITY] ⚠️ ALERT: ${result.reason}`);
      console.error(`Command: ${command.slice(0, 100)}`);
      console.log(JSON.stringify({ continue: true }));
      break;

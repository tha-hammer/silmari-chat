...
import { captureFailure } from '../AAI/Tools/FailureCapture';
...
function getCurrentWorkSlug(): string | undefined {
  try {
    const stateDir = join(BASE_DIR, 'MEMORY', 'STATE');
    // Try session-scoped first, then legacy
    const sessionId = process.env.CLAUDE_SESSION_ID;
    const files = sessionId
      ? [join(stateDir, `current-work-${sessionId}.json`), join(stateDir, 'current-work.json')]
      : [join(stateDir, 'current-work.json')];
    for (const f of files) {
      if (existsSync(f)) {
        const state = JSON.parse(readFileSync(f, 'utf-8'));
        if (state.session_dir) return state.session_dir;
      }
    }
  } catch {}
  return undefined;
...
async function main() {
  try {
    ...
    if (explicitResult) {
      ...
      // === v4.1.0: Emit canonical event (beads indexing via bus consumer) ===
      try {
        const { getEventBus } = await import('./lib/event-bus');
        const activeSlug = getCurrentWorkSlug();
        getEventBus().emit({
          type: 'signal.captured',
          sessionId: data?.session_id || process.env.CLAUDE_SESSION_ID || '',
          rating: entry.rating,
          sentiment: entry.source || 'explicit',
          summary: entry.sentiment_summary || '',
          workSlug: activeSlug,
        });
      } catch { /* Silent */ }
    ...
    try {
      ...
      try {
        ...
        getEventBus().emit({
          ...
          sessionId: data?.session_id || process.env.CLAUDE_SESSION_ID || '',
          rating: entry.rating,
          sentiment: entry.source || 'implicit',
          summary: entry.sentiment_summary || '',
          workSlug: activeSlug,
        });
      } catch { /* Silent */ }

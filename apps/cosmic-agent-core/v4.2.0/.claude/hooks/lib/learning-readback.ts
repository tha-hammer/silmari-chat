 ...
 *
 * PURPOSE:
 * The AAI learning system writes extensively (8,400+ files across 5 hooks) but
...
export function loadLearningDigest(paiDir: string): string | null {
  ...
  try {
    ...
    if (isBeadsAvailable()) {
      const algoItems = brList({ labels: ['kind:learning', 'learning:algorithm'], limit: 3, sort: 'created_at', reverse: true });
      const sysItems = brList({ labels: ['kind:learning', 'learning:system'], limit: 3, sort: 'created_at', reverse: true });
      if (algoItems.length > 0 || sysItems.length > 0) {
        const parts: string[] = ['**Recent Learning Signals:**'];
        if (algoItems.length > 0) {
          parts.push('*Algorithm:*');
          algoItems.forEach((i: any) => parts.push(`  ${i.title.slice(0, 80)}`));
        }
        if (sysItems.length > 0) {
          parts.push('*System:*');
          sysItems.forEach((i: any) => parts.push(`  ${i.title.slice(0, 80)}`));
        }
        return parts.join('\n');
      }
    }
  ...
  // === Filesystem fallback (v4.0.3 behavior) ===
  ...
  try {
    ...
    if (isBeadsAvailable()) {
      ...
      if (failures.length > 0) {
        const patterns = failures.map((f: any) => {
          const date = f.created_at?.slice(0, 10) || '?';
          return `[${date}] ${f.title.slice(0, 70)}`;
        });

/**
 * change-detection.ts - Utilities for detecting AAI system changes
 *
...
const STATE_FILE = join(AAI_DIR, 'MEMORY', 'STATE', 'integrity-state.json');
...
/**
 * Normalize an absolute path to relative (to AAI_DIR).
 */
function normalizeToRelativePath(absolutePath: string): string {
  if (absolutePath.startsWith(AAI_DIR)) {
    return relative(AAI_DIR, absolutePath);
  }
  return absolutePath;
}
...
/**
 * Categorize a file path by its location in the AAI system.
 */
export function categorizeChange(path: string): ChangeCategory | null {
  ...
  // Check if path is within AAI directory
  const absolutePath = path.startsWith('/') ? path : join(AAI_DIR, path);
  if (!absolutePath.startsWith(AAI_DIR)) {
    return null;
  }
  ...
  if (path.includes('skills/')) {
    ...
    if (path.match(/AAI\/(?:AAISYSTEM|THEHOOKSYSTEM|THEDELEGATION|MEMORYSYSTEM|AISTEERINGRULES)/)) return 'core-system';
    return 'skill';
  }

  if (path.includes('hooks/')) return 'hook';
  if (path.includes('MEMORY/AAISYSTEMUPDATES/')) return 'documentation';
...
export function isSignificantChange(changes: FileChange[]): boolean {
  // Filter to only AAI system changes
...
export function generateDescriptiveTitle(changes: FileChange[]): string {
  ...
  for (const p of paths) {
    ...
    if (match && match[1] !== 'AAI') skillNames.add(match[1]);
  }
  ...
  const hasCoreSystem = paths.some(p => p.match(/AAI\/(?:AAISYSTEM|THEHOOKSYSTEM|THEDELEGATION|MEMORYSYSTEM|AISTEERINGRULES)/));
  const hasCoreUser = paths.some(p => p.includes('AAI/USER/'));
  ...
  // Core system changes
    ...
    const docNames = paths
      .filter(p => p.match(/AAI\/(?:AAISYSTEM|THEHOOKSYSTEM|THEDELEGATION|MEMORYSYSTEM|AISTEERINGRULES)/))
      .map(p => basename(p, '.md'));
    if (docNames.length === 1) {
      title = `${docNames[0]} Documentation Updated`;
    } else {
      title = 'AAI System Documentation Updated';
    }
  }
  // Core user changes
  else if (hasCoreUser) {
    const docNames = paths
      .filter(p => p.includes('AAI/USER/'))
      .map(p => basename(p, '.md'));
    if (docNames.length === 1) {

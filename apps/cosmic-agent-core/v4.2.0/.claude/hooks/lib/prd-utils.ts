...
export function findLatestPRD(): string | null {
  // === v4.1.0: Beads-first path ===
  try {
    const { isBeadsAvailable, brList } = require('./beads-index');
    if (isBeadsAvailable()) {
      const items = brList({ labels: ['note:artifact', 'kind:prd'], status: 'open', limit: 1, sort: 'updated_at', reverse: true });
      if (items.length > 0) {
        const pathMatch = items[0].description?.match(/path:\s*(.+)/);
        if (pathMatch) {
          const prdPath = paiPath(pathMatch[1].trim());
...
export function syncToWorkJson(fm: Record<string, string>, prdPath: string, content?: string, sessionId?: string): void {
  ...
  // === v4.1.0: Beads dual-write ===
  try {
    const { indexWorkItem, indexCriteriaFragments } = require('./beads-index');
    const beadsId = indexWorkItem(fm, prdPath, sessionId);
    if (beadsId && content) {
      indexCriteriaFragments(beadsId, content);
    }
  } catch {
    // Silent — beads is never blocking
  }
  // === End beads dual-write ===
...
export function updateSessionNameInWorkJson(sessionUUID: string, sessionName: string): void {
  // === v4.1.0: Beads dual-write ===
  try {
    const { isBeadsAvailable, brList, brUpdate } = require('./beads-index');
    if (isBeadsAvailable()) {
      const results = brList({ labels: ['note:state', 'kind:session-summary'], descContains: sessionUUID, limit: 1 });
      if (results && results.length > 0) {
        brUpdate(results[0].id, { title: sessionName });
      }
    }
  } catch { /* Silent */ }
  // === End beads dual-write ===
...
export function upsertSession(sessionUUID: string, sessionName: string, task: string, mode: 'native' | 'starting' = 'native'): void {
  // === v4.1.0: Beads dual-write ===
  try {
    const { isBeadsAvailable, brCreate, brUpdate, brList } = require('./beads-index');
    if (isBeadsAvailable()) {
      const labels = 'note:state,kind:session-summary,memory:work';
      const existing = brList({ labels: ['note:state', 'kind:session-summary'], descContains: sessionUUID, limit: 1 });
      if (existing && existing.length > 0) {
        brUpdate(existing[0].id, { notes: `session: ${sessionUUID}\nupdated: ${new Date().toISOString()}` });
      } else {
        const id = brCreate({ title: sessionName || task || 'Session', type: 'task', labels, description: `session: ${sessionUUID}` });
        if (id) brUpdate(id, { notes: `session: ${sessionUUID}` });
      }
    }
  } catch { /* Silent */ }
  // === End beads dual-write ===

 ...
 *
 * Detects AAI system changes from the transcript and spawns background
...
const INTEGRITY_SCRIPT = paiPath('AAI', 'Tools', 'IntegrityMaintenance.ts');
 ...
 * 2. Filters for AAI system paths (excludes WORK/, LEARNING/)
...
export async function handleSystemIntegrity(
  ...
  console.error(`[SystemIntegrity] ${systemChanges.length} are AAI system changes`);

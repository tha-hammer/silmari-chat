...
import type { ParsedTranscript } from '../../AAI/Tools/TranscriptParser';
...
function isSystemDocModified(modifiedFiles: Set<string>): boolean {
  for (const path of modifiedFiles) {
    if (path.includes('AAI/') && path.endsWith('.md')) return true;
  }
  return false;
}
...
/**
 * Check if ANY meaningful AAI system file was modified.
...
function isSystemFileModified(modifiedFiles: Set<string>): boolean {
  ...
  for (const filePath of modifiedFiles) {
    ...
    const relPath = filePath.startsWith(AAI_DIR)
      ? filePath.slice(AAI_DIR.length + 1)
      : filePath;

    // Must be within AAI directory
    if (filePath.startsWith('/') && !filePath.startsWith(AAI_DIR)) continue;

    // Skip excluded paths
    if (EXCLUDED.some(ex => relPath.includes(ex))) continue;

    // Match meaningful system files (AAI root MDs, SYSTEM docs, USER dir, Algorithm, Tools, Workflows, skills)
    if ((relPath.startsWith('AAI/') || relPath.includes('skills/')) && (relPath.endsWith('.md') || relPath.endsWith('.ts') || relPath.endsWith('.yaml') || relPath.endsWith('.yml'))) return true;
    if (relPath.includes('hooks/') && relPath.endsWith('.ts')) return true;
    if (relPath.endsWith('settings.json')) return true;
    if (relPath.includes('AAI/Algorithm/') && relPath.endsWith('.md')) return true;
...
function checkSystemDocRefs(docsToCheck: string[], systemDocsOnDisk: Set<string>): DriftItem[] {
  const drift: DriftItem[] = [];
  // Match backtick-wrapped or plain doc references in AAI/ (both old skills/AAI/ and new AAI/ paths)
  const sysDocRefRegex = /(?:`|'|")(?:~\/\.claude\/)?(?:skills\/)?AAI\/([\w/]+\.md)(?:`|'|")/g;
  ...
  for (const docFile of docsToCheck) {
    ...
    while ((match = sysDocRefRegex.exec(content)) !== null) {
      const refTarget = match[1]; // e.g., "AAISYSTEMARCHITECTURE.md" or "AAISECURITYSYSTEM/ARCHITECTURE.md"
      const targetBasename = basename(refTarget);
      // For nested paths like AAISECURITYSYSTEM/ARCHITECTURE.md, check file exists
      const targetPath = join(SYSTEM_DIR, refTarget);
      if (!existsSync(targetPath)) {
        drift.push({
          doc: docFile,
          pattern: 'system_doc_ref',
          reference: `AAI/${refTarget}`,
          issue: `References "AAI/${refTarget}" but file does not exist`,
        });
      }
    }
  }
...
export async function handleDocCrossRefIntegrity(
  ...
  // Run if ANY meaningful AAI system file was modified (skills, hooks, tools, config, components, workflows, SYSTEM docs)
  ...
  for (const path of modifiedFiles) {
    if (path.includes('AAI/') && path.endsWith('.md')) {
